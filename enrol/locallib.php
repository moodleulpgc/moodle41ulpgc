...
class course_enrolment_manager {
    ...
    public function get_other_users($sort, $direction='ASC', $page=0, $perpage=25) {
        ...
        if (!array_key_exists($key, $this->otherusers)) {
            list($ctxcondition, $params) = $DB->get_in_or_equal($this->context->get_parent_context_ids(true), SQL_PARAMS_NAMED, 'ctx');
            if($nolistroles = get_config('local_ulpgccore', 'nolistroles') && !has_capability('local/ulpgccore:manage', $this->context)) {
                list($rolecondition, $rparams) = $DB->get_in_or_equal(explode($nolistroles), SQL_PARAMS_NAMED, 'ctx', false); // ecastro ULPGC to exclude roles from listing
                $params = $params + $rparams;
                $ctxcondition .= ' AND ra.roleid '.$rolecondition;
            }
            $params['courseid'] = $this->course->id;

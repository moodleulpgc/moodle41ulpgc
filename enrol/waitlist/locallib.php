...
class enrol_waitlist_enrol_form extends moodleform {
    ...
    public function definition() {
		...
		if($isDisabled){
			...
			$do = true;
			if(($instance->customint6 !=0) && waitlist_user_max_enrolments($USER->id, $instance)) { // ecastro ULPGC
                $mform->addElement('html', get_string('maxcoursesfull', 'enrol_waitlist', $instance->customint6));
                $do= false;
            } elseif($instance->customint3==0){
                $mform->addElement('html', get_string('confirmation', 'enrol_waitlist'));
            } elseif($enroledCount<$instance->customint3) {
                $mform->addElement('html', get_string('confirmation', 'enrol_waitlist'));
            } elseif($instance->customint5) {
                ...
                $mform->addElement('html', get_string('confirmationfullno', 'enrol_waitlist')); // ecastro ULPGC
                $do= false;
            }
...
function waitlist_user_max_enrolments($userid, $instance) { // ecastro ULPGC
    global $DB;
    ...
    $sql = "SELECT COUNT(DISTINCT(e.courseid))  
            ...
            WHERE e.enrol = 'waitlist' AND e.status = 0 AND c.category = (SELECT c2.category FROM {course} c2 WHERE c2.id = :courseid) ";
    
    $count =  $DB->count_records_sql($sql, array('userid'=>$userid, 'courseid'=>$instance->courseid));
    if($count >= $instance->customint6) {
        return true;
    }
    return false;
}

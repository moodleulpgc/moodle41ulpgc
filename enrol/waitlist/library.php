...
class course_enrolment_manager {
    ...
    public function get_users($sort, $direction='ASC', $page=0, $perpage=25,$instance) {
        ...
        if (!array_key_exists($key, $this->users)) {
            ...
            if(array_search('email', $extrafields) === false) { // ecastro ULPGC
                $extrafields[] = 'email';
            }
     ...
     * @return array
     */
    public function get_users_for_display(course_enrolment_manager $manager, $sort, $direction, $page, $perpage, $instance) {
        $pageurl = $manager->get_moodlepage()->url;
        $users = $this->get_users($sort, $direction, $page, $perpage, $instance);

...
class restore_groups_structure_step extends restore_structure_step {
    ...
    public function process_group($data) {
        ...
        // ecastro ULPGC
        $newitemid = 0;
        $groupbyidnumber =  $this->get_setting_value('groupbyidnumber');
        if($groupbyidnumber && $data->idnumber) {
            if ($group = groups_get_group_by_idnumber($data->courseid, $data->idnumber)) {
                // group exists, use it
                $newitemid = $group->id;         
                // Save the id mapping
        ...
        if (isset($data->idnumber) and has_capability('moodle/course:changeidnumber', $context, $this->task->get_userid())) {
            if (groups_get_group_by_idnumber($data->courseid, $data->idnumber)) {
                //unset($data->idnumber);
                $data->idnumber .= '_restored_'. $this->task->get_restoreid(); // ecastro ULPGC
            }
        } else {
            unset($data->idnumber);
        }
    ...
    public function process_grouping($data) {
        ...
        $groupbyidnumber =  $this->get_setting_value('groupbyidnumber');
        if($groupbyidnumber && $data->idnumber) {
            if ($grouping = groups_get_grouping_by_idnumber($data->courseid, $data->idnumber)) {
                // group exists, use it
                $newitemid = $grouping->id;         
                // Save the id mapping

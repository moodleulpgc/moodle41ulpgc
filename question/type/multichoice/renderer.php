...
abstract class qtype_multichoice_renderer_base extends qtype_with_combined_feedback_renderer {
    ...
    public function formulation_export(question_attempt $qa, question_display_options $options) { // ecastro ULPGC
        ...
        foreach ($question->get_order($qa) as $value => $ansid) {
            ...
            $number = $this->number_in_style($value, $question->answernumbering);
            $number = str_replace('.', ')', $number);
            $choice = $question->make_html_inline($question->format_text($ans->answer, $ans->answerformat,
                                                                        $qa, 'question', 'answer', $ansid));
            $right = '';
            if($options->rightanswer && $this->is_right($ans)) {
                $right = '&#169; ';
            }
            $choices[] = $number.$right.$choice;
        }
        $choicestext = html_writer::start_tag('div', array('class' => 'answer', 'style'=>'text-indent: 5.0em; '));
        $choicestext .= implode('<br />', $choices);
        $choicestext .= html_writer::end_tag('div');
...
class qtype_multichoice_single_renderer extends qtype_multichoice_renderer_base {
    ...
    protected function is_right(question_answer $ans) {
        return ($ans->fraction > 0 ); // ecastro ULPGC return only bool / int
    }

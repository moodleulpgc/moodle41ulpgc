    ...
    events.forEach((event) => {
        document.addEventListener(event, async(e) => {
            if (e.target.closest(selectors.elements.sectionmodchooser)) {
                ...
                //ecastro ULPGC add strings for chooser groups
                const enabledadvchooser = parseInt(chooserConfig.enabledadvchooser);
                if (enabledadvchooser) {
                    chooserConfig.names = {};
                    for(var group in chooserConfig.groups) {
                        chooserConfig.names[group] = await getString(group, 'local_ulpgccore');
                    }
                }
                //ecastro ULPGC add strings for chooser groups
...
const templateDataBuilder = (data, chooserConfig) => {
    ...
    if (enabledadvchooser) {
        ...
        // to keep track of grouped & non-grouped mods
        remaining = data;
        for(var group in chooserConfig.groups) {
            let groupmods = [];
            let filtered = [];

            groupmods = chooserConfig.groups[group];
            filtered = data.filter(mod => groupmods.includes(mod.name));
            // removes grouped mods
            remaining = remaining.filter(mod => (groupmods.includes(mod.name) == false));
            if (filtered && filtered.length > 0) {
                grouped = {'label' : chooserConfig.names[group],   'content' : filtered};
                allgrouped.push(grouped);
                if(group.includes('res_')) {
                    res.push(grouped);
                }
                if(group.includes('actv_')) {
                    actv.push(grouped);
                }
            }
        }
        ...
        if (remaining && remaining.length > 0) {
            grouped = {'label' : 'Otros ítems',   'content' : remaining};
            allgrouped.push(grouped);
        }
    ...
    return {
        ...
        fallback: fallback,
        advchooser: enabledadvchooser, // ecastro ULPGC
    };
};

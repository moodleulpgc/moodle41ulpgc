...
function update_moduleinfo($cm, $moduleinfo, $course, $mform = null) {
    ...
    if(isset($moduleinfo->score)) {
        $cm->score = $moduleinfo->score; // ecastro ULPGC to enforce admin restricted mods
    }
...
function get_moduleinfo_data($cm, $course) {
    ...
    $data->score              = (isset($cm->score) && $cm->score) ? 1 : 0 ; // ULPGC ecastro to enforce admin mods
    if (!empty($CFG->enableavailability)) {
...
function prepare_new_moduleinfo_data($course, $modulename, $section) {
    ...
    $data->instance         = '';
    $data->coursemodule     = '';
    $data->score            = 0; // ULPGC ecastro to enforce admin mods

...
abstract class moodleform_mod extends moodleform {
    ...
    protected function standard_coursemodule_elements() {
        ...
        if (get_config('local_ulpgccore', 'enabledadminmods') && has_capability('local/ulpgccore:manage', context_system::instance())) {  // ULPGC ecastro institutional unerasable adminmods
            $mform->addElement('advcheckbox', 'score', get_string('adminmoduleoptions', 'local_ulpgccore'), get_string('adminmoduleexplain', 'local_ulpgccore'));
            $mform->setType('score', PARAM_INT);
        } else {
            $mform->addElement('hidden', 'score', 0);
            $mform->setType('score', PARAM_INT);            
            $mform->disabledIf('cmidnumber', 'score', 'eq', 1); 
        }

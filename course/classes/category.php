...
class core_course_category implements renderable, cacheable_object, IteratorAggregate {
     ...
     * @param bool $short whether long hierarchical paths should be shortened  // ecastro ULPGC  
     * @return array of strings
     */
    public static function make_categories_list($requiredcapability = '', $excludeid = 0, $separator = ' / ', $short = true) {
        global $DB;
        $coursecatcache = cache::make('core', 'coursecat');
        ...
        // ULPGC ecastro to cut short category names in selects
        if($short) {
            foreach ($names as $key => $value) {
                // Prefix the value with the number of spaces equal to category depth (number of separators in the value).
                $names[$key] = str_repeat('&#160;', substr_count($value, ' / ')). $value;
            }
            //ecastro ULPGC, replace &nbsp; by &#160;
            foreach($names as $key =>$cat) {     
                if($name = strrchr($cat, '/')) {
                    $n = substr_count($cat, '/');
                    $names[$key] = str_repeat('&#160;', $n-1).'&#160;'.$name;
                }
            }        
        }
        // ULPGC ecastro to cut short category names in selects
    ...
    public function can_resort_courses() {
        return ($this->has_manage_capability() || has_capability('local/ulpgccore:categoryreview', $this->get_context())) && 
                    $this->coursecount == $this->get_courses_count(); // ecastro ULPGC allow courses resort in non-editable category
    }

...
class core_course_management_renderer extends plugin_renderer_base {
    ...
    public function category_listitem(core_course_category $category, array $subcategories, $totalsubcategories,
        ...
        $checkboxclass = '';
        if (!$category->can_resort_subcategories() && !$category->has_manage_capability() && !$category->can_resort_courses() ) { // ecastro ULPGC category review sorting
        ...
        $hasactions = !empty($actions) || $category->can_create_course() || $category->can_resort_courses(); // ecastro ULPGC category review sorting
    ...
    public function category_bulk_actions(core_course_category $category = null) {
        // Resort courses.
        // Change parent.
        if (!core_course_category::can_resort_any() && !core_course_category::can_change_parent_any() && ($category && !$category->can_resort_courses())) {  // ecastro ULPGC
            return '';
        }
        ...
        if (core_course_category::can_resort_any() || ($category && $category->can_resort_courses())) { // ecastro ULPGC
            $selectoptions = array(
                'selectedcategories' => get_string('selectedcategories'),
                'allcategories' => get_string('allcategories')
            );
            if(!$canchange = core_course_category::can_resort_any()) { // ecastro ULPGC allow courses resort in non-editable category
                $selectoptions = array();
            ...
            if(!$canchange) { // ecastro ULPGC allow courses resort in non-editable category
                $form .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'resortcategoriesby', 'value' => 'none'));
            } else {
                $form .= html_writer::div(
                    html_writer::select(
                        array(

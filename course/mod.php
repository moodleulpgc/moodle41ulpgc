...
$ulpgc = get_config('local_ulpgccore', 'enabledadminmods'); // ecastro ULPGC to enforce adminmods
...
if (!empty($add)) {
    ...
    if($ulpgc && $cm->score) {
        require_capability('local/ulpgccore:modedit', context_module::instance($cm->id)); // ecastro ULPGC enforce adminmods non duplicate
    }
    $returntomod = optional_param('return', 0, PARAM_BOOL);
    ...
    if($ulpgc && $cm->score) {
        require_capability('local/ulpgccore:modduplicate', $modcontext); // ecastro ULPGC enforme adminmods non duplicate
    }
    ...
    require_capability('moodle/course:manageactivities', $modcontext);
    if($ulpgc && $cm->score) {
        require_capability('local/ulpgccore:moddelete', $modcontext); // ecastro ULPGC enforme adminmods non delete
    }
...
if ((!empty($movetosection) or !empty($moveto)) and confirm_sesskey()) {
    ...
    require_capability('moodle/course:manageactivities', $modcontext);
    if($ulpgc && $cm->score) {
        require_capability('local/ulpgccore:modmove', $modcontext); // ecastro ULPGC enforme adminmods non move
    }

...
function can_edit_in_category($categoryid = 0) {
    ...
    // ecastro ULPGC
    if(get_capability_info('local/ulpgccore:categoryreview')) {
        $cap = array('local/ulpgccore:categoryreview'); // ecastro ULPGC to allow category review & course sorting by coordinators
    }
    return has_any_capability(array('moodle/category:manage', 'moodle/course:create', 'local/ulpgccore:categoryreview') + $cap, $context);
}
...
function course_get_cm_edit_actions(cm_info $mod, $indent = -1, $sr = null) {
    ...
    // adminmods restrictions // ecastro ULPGC
        $adminmods = get_config('local_ulpgccore', 'enabledadminmods');  
    ...
    // Update.
    if ($hasmanageactivities && $hasmodedit) { // ecastro ULPGC adminmods enforce modedit
    ...
    if (has_all_capabilities($dupecaps, $coursecontext) &&
            ...
            course_allowed_module($mod->get_course(), $mod->modname) && $hasmodduplicate) { // ecastro ULPGC adminmods enforce modduplicate
    ...
    // Assign.
    if (has_capability('moodle/role:assign', $modcontext) && $hasmodroles){ // ecastro ULPGC adminmods enforce modduplicate
    ...
    // Delete.
    if ($hasmanageactivities && $hasmoddelete) { // ecastro ULPGC adminmods enforce modedit
...
function course_get_cm_move(cm_info $mod, $sr = null) {
    ...
    $modcontext = context_module::instance($mod->id);
    $hasmanageactivities = has_capability('moodle/course:manageactivities', $modcontext);
    $hasmodmove = (get_config('local_ulpgccore', 'enabledadminmods') && $mod->score) ? has_capability('local/ulpgccore:modmove', $modcontext): true; // ecastro ULPGC adminmods to enforce modmove
    ...
    if ($hasmanageactivities && $hasmodmove) { // ecastro ULPGC adminmods to enforce modmove
        $pixicon = 'i/dragdrop';

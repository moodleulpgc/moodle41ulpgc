...
class course_edit_form extends moodleform {
    ...
    function definition_after_data() {
        ...
        /// ecastro ULPGC freeze unchangeable fields for teacher no coordinator
        if($courseid) { 
            $coursecontext = context_course::instance($courseid);
            $course        = $this->_customdata['course'];
            if(!has_capability('moodle/course:changesummary', $coursecontext)) { // ecastro ulpgc limit editable course form fields
                $frozenfields = array('startdate', 'visible', 'format', 'coursedisplay', 'groupmode', 'groupmodeforce', 'defaultgroupingid');
                if($instances = enrol_get_instances($courseid, false)) {
                        foreach($instances  as $instance) {
                            $frozenfields[] = 'enrol_guest_status_'.$instance->id;
                            $frozenfields[] = 'enrol_guest_password_'.$instance->id;
                        }
                    }
                foreach($frozenfields as $field) {
                    if($mform->elementExists($field)) {
                        $mform->hardFreeze($field);
                        $mform->setConstants($field, $course->$field);
                    }
                }
            }
        }

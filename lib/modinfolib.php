...
class cm_info implements IteratorAggregate {
    ...
    public $recentactivity = false; // ecastro ULPGC
    public $adminmods = false; // ecastro ULPGC 
    ...
    public function __construct(course_modinfo $modinfo, $notused1, $mod, $notused2) {
        ...
        $this->recentactivity = get_config('local_ulpgccore', 'enabledrecentactivity'); // ecastro ULPGC
        $this->adminmods = get_config('local_ulpgccore', 'enabledadminmods');  // ecastro ULPGC
        $score = 0;
        if($this->adminmods) {
            global $DB;
            $score = $DB->get_field('course_modules', 'score', array('id'=>$mod->cm)); // ecastro ULPGC to enforce admin restricted modules
        }
        $this->score = ((isset($mod->score) && $mod->score) || $score) ? 1 : 0;

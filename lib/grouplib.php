...
function groups_get_all_groups($courseid, $userid=0, $groupingid=0, $fields='g.*', $withmembers=false) {
        ...
        if(!$allowed) { // ecastro ULPGC
            foreach($groups as $gid => $group) {
                if(($group->idnumber == 'C7') || ($group->idnumber == 'C56') || ($group->idnumber == 'PyR') 
                    || ($group->idnumber == 'PRG') || ($group->idnumber == 'RET')) {
                    unset($groups[$gid]);
                }
            }
        }
    ...
    if(!$allowed) {
        $unallowedwhere = " AND ((g.idnumber != 'C7') AND (g.idnumber != 'C56') 
                            AND (g.idnumber != 'PyR') AND (g.idnumber != 'PRG') AND (g.idnumber != 'RET'))";
    }
...
function groups_print_activity_menu($cm, $urlroot, $return=false, $hideallparticipants=false) {
    ...
    if ($separateallgroups && !isset($groupsmenu[0]) && !$hideallparticipants) {

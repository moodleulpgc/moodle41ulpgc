 ...
 * @version    $Id: Subset.php 1407 2020-01-28 08:56:29Z jan.slabon $
 */
    ...
    extends SetaPDF_Core_Font_TrueType_Subset
{
    ...
    {
        $this->_writeCidSet = (boolean)$writeCidSet;
    }
    ...
    protected function _createCidSet()
    {
         ...
         */
        $glyf = $this->_getSubsetFile()->getTable(SetaPDF_Core_Font_TrueType_Table_Tags::GLYF);
        $count = $glyf->getNumGlyphs();
    ...
    protected function _createCidToGidMap()
    {
        ...
        $streamData = '';
        foreach ($this->_cidToGlyphId as $cid => $gid) {
            $streamData .= $gid;
        }
    ...
    protected function _createSubset()
    {
        ...
        if ($this->_writeCidSet) {
            $fontDescriptorData['CIDSet'] = $this->_document->createNewObject($this->_createCidSet());
        }

        $fontDescriptor = new SetaPDF_Core_Type_Dictionary($fontDescriptorData);

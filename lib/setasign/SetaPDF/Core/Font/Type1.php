 ...
 * @version    $Id: Type1.php 1560 2020-09-24 12:17:20Z maximilian.kresse $
 */
    ...
    implements SetaPDF_Core_Font_Glyph_Collection_CollectionInterface, SetaPDF_Core_Font_DescriptorInterface
{
    ...
    public function __construct($indirectObjectOrDictionary)
    {
        ...
        if (!$firstCharExists || !$lastCharExists) {
            $widthsCount = count($dictionary->offsetGet('Widths')->ensure());
            if (!$firstCharExists && !$lastCharExists && $widthsCount === 256) {
                ...
                $lastChar = $dictionary->offsetGet('LastChar')->ensure()->getValue();
                $dictionary['FirstChar'] = new SetaPDF_Core_Type_Numeric($lastChar - $widthsCount + 1);
            } elseif ($firstCharExists && !$lastCharExists) {
                $firstChar = $dictionary->offsetGet('FirstChar')->ensure()->getValue();

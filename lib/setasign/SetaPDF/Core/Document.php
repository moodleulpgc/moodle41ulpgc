 ...
 * @version    $Id: Document.php 1547 2020-09-17 15:04:01Z maximilian.kresse $
 */
    ...
    implements SplObserver, SetaPDF_Core_WriteInterface, SetaPDF_Core_Type_Owner
{
     ...
     * @var string
     */
    const CACHE_ICC_PROFILE = 'ICCProfile';
     ...
     * @var string
     */
    const CACHE_ENCAPSULATED_CONTENT_STREAMS = 'EncapsulatedContentStream';
     ...
     * @var string
     */
    const CACHE_FONT_DESCRIPTOR = 'FontDescriptor';
     ...
     * this will be an instance of {@link SetaPDF_Core_Parser_CrossReferenceTable}
     *
     ...
     * @var array
     */
    protected $_blockedReferencedObjects = [];
     ...
     * @var array
     */
    protected $_beforeSaveCallbacks = [];
    ...
    protected $_cache = [
        self::CACHE_FONT => [],
        self::CACHE_X_OBJECT => [],
        self::CACHE_ICC_PROFILE => [],
        self::CACHE_ENCAPSULATED_CONTENT_STREAMS => [],
        self::CACHE_FONT_DESCRIPTOR => []
    ];
     ...
     * @return SetaPDF_Core_Document Returns a {@link SetaPDF_Core_Document} instance
    ...
    )
    {
        ...
        $instance->_maxObjId = $instance->_xref->getSize() - 1;
     ...
     *
     * You can use the methods from {@link SetaPDF_Core_Document_Catalog::getDocumentMagicMethods()}.
     *
    ...
    {
        // Catalog
        if (in_array($method, SetaPDF_Core_Document_Catalog::getDocumentMagicMethods())) {
            return call_user_func_array([$this->getCatalog(), $method], $arguments);
        }
    ...
    public function addIndirectObjectReferenceWritten(SetaPDF_Core_Type_IndirectObjectInterface $indirectObject)
    {
        ...
        if (
            ...
            $this->_referencedObjects[$objectIdent] = [
    ...
    public function getIdForObject(SetaPDF_Core_Type_IndirectObjectInterface $indirectObject)
    {
        ...
        if ($this->_instanceIdent === $indirectObject->getOwnerPdfDocument()->getInstanceIdent()) {
            $data = [$indirectObject->getObjectId(), $indirectObject->getGen()];
        } else {
            $this->getXref()->updateSize(++$this->_maxObjId);
            $data = [$this->_maxObjId, 0];
        }
    ...
    public function cloneIndirectObject(SetaPDF_Core_Type_IndirectObject $indirectObject)
    {
        ...
        $this->_objectsToIds[$objectIdent] = [$object->getObjectId(), $object->getGen()];
    ...
    public function deleteObject(SetaPDF_Core_Type_IndirectObjectInterface $object)
    {
        ...
        $this->_deletedObjects[$objectData[0]] = $objectData[1];
    }
     ...
     * {@link SetaPDF_Core_Document::SAVE_METHOD_REWRITE} to it. This task is very performance intensive, because the
     ...
     * {@link SetaPDF_Core_Document::SAVE_METHOD_REWRITE_ALL}. The disadvantage is, that unused objects may be
     * copied/written, too.
     *
    ...
    public function save($method = true)
    {
        ...
        foreach ($this->_beforeSaveCallbacks AS $name => $callback) {
            call_user_func_array($callback, [$this, $name]);
        }
        ...
        if (
            ($this->_secHandlerIn === null && $this->_secHandler !== null) ||
            ($this->_secHandlerIn !== null && $this->_secHandler === null) ||
            (
                $this->_secHandler !== null && $this->_secHandlerIn !== null &&
            ...
            } else if ($this->_trailer->offsetExists('Encrypt')) {
            ...
            if ($this->_trailer->offsetExists('Root')) {
                ...
                if ($root->offsetExists('Metadata')) {
                    ...
                    if (!($filters->getValue() instanceof SetaPDF_Core_Type_Array)) {
                        $filters->setValue(new SetaPDF_Core_Type_Array([$filters->getValue()]));
                    }
        ...
        if (
            $method === self::SAVE_METHOD_UPDATE &&
        ...
        if (self::SAVE_METHOD_UPDATE === $method &&
            ...
            if (count($this->_writeFileBody()) > 0 || $this->_trailerChanged || $this->_xref->offsetsUpdated()) {
            ...
            $updatedAndWrittenObjects = $this->_writeFileBody();
            ...
            if ($this->_parser instanceof SetaPDF_Core_Parser_Pdf) {
                ...
                if (!$rootExists || $this->_saveMethod === self::SAVE_METHOD_REWRITE_ALL) {
                    ...
                    for ($i = 1; $i < $maxObjId; $i++) {
                        ...
                        if (
                            isset($updatedAndWrittenObjects[$i]) ||
                            isset($outdatedXrefObjects[$i]) ||
                            isset($this->_deletedObjects[$i])
                        ) {
                            continue;
                        }
            ...
            if (!$objectsResolved) {
                foreach ($this->_trailer AS $key => $entry) {
                    if ($entry instanceof SetaPDF_Core_Type_IndirectReference) {
                        ...
                        if ($key === 'Encrypt' && $this->_secHandler === null) {
                            continue;
                        }
    ...
    protected function _writeObject(SetaPDF_Core_Document $document, $objectId, $generation, $cache)
    {
        if ($this->_compressXref && $this->_saveMethod === self::SAVE_METHOD_REWRITE_ALL) {
            ...
            /** @var SetaPDF_Core_Parser_CrossReferenceTable $xref */
            $xref = $this->_xref;
            $offset = $xref->getParserOffsetFor($objectId, $generation);
            ...
            if (!is_int($offset[0])) {
                ...
                if (!array_key_exists($streamKey, $this->_objectStreams)) {
                    try {
                        /** @var SetaPDF_Core_Type_ObjectStream $streamObject */
                        $streamObject = $this->resolveIndirectObject($offset[0][0], $offset[0][1]);
                    } catch (SetaPDF_Core_Document_ObjectNotDefinedException $e) {
                        throw new SetaPDF_Core_Document_ObjectNotFoundException(
                            sprintf('Cannot resolve object (%s, %s)', $offset[0][0], $offset[0][1])
                    ...
                    $streamObject = $this->_objectStreams[$streamKey];
                }

                $streamObject = $streamObject->ensure();
                if (
                    $streamObject->getOwnerPdfDocument()->getInstanceIdent() === $this->getInstanceIdent() &&
                    $streamObject->isValid()
                ) {
                    return;
                }
            }
         ...
         */
        if (
            $this->_compressXref && $this->_saveMethod === self::SAVE_METHOD_REWRITE_ALL &&
            $object->ensure() instanceof SetaPDF_Core_Type_ObjectStream
        ) {
            /** @var SetaPDF_Core_Type_ObjectStream $objectStream */
            $objectStream = $object->ensure();

            if ($objectStream->isValid() === false &&
            ...
            foreach (array_keys($objectStream->getOffsets()) AS $index => $_objectId) {
                if ($this->_xref->isOffsetUpdated($_objectId) === false) {
                    $this->_xref->setOffsetFor($_objectId, 0, [$data[0], $index]);
                } else {
                    // if an object was already written in another object stream check
                    $a = $this->resolveIndirectObject($_objectId, 0, false)->toPhp();
                    $b = $objectStream->resolveIndirectObject($_objectId)->toPhp();
    ...
    protected function _writeFileBody()
    {
        ...
        if (is_callable($this->_fileBodyMethod)) {
            return call_user_func($this->_fileBodyMethod, $this);
        }
    ...
    protected function _updateFileIdentifier()
    {
        ...
        if (!$this->_trailer->offsetExists('ID')) {
            $this->_trailer['ID'] = new SetaPDF_Core_Type_Array([
    ...
    public function getFileIdentifier($permanent = false, $create = true)
    {
        if (!$this->_trailer->offsetExists('ID')) {
            if ($create) {
                // If a document is encrypted and no file identifier is set, we need to create one with empty strings.
                if ($this->getSecHandlerIn() !== null) {
                    $this->setNewFileIdentifier('');
                }
    ...
    {
        $objectsWritten = [];
        while (($object = array_pop($this->_changedObjects)) !== null) {
            $this->writeObject($object);
            $objectData = $this->getIdForObject($object);
            $objectsWritten[$objectData[0]] = $objectData[1];
        }
    ...
    {
        if ($this->_useWriteCallbacks === false) {
            return $value;
        }
    ...
    /**
     * Implementation of the {@link SetaPDF_Core_Type_Owner} interface.
     *
     * @return $this
     */
     ...
     * @param mixed $value
     */
    public function addCache($type, $name, $value)
    {
        if (!isset($this->_cache[$type])) {
            throw new InvalidArgumentException(sprintf('Unknown cache type "%s"', $type));
        }
    ...
    /**
     * Get a cahce item by its type and name.
     *
     * @param string $type
     * @param string $name
     * @return mixed
    ...
    {
        if (!isset($this->_cache[$type][$name])) {
            throw new InvalidArgumentException(sprintf('Unknown cache name "%s" for cache type "%s".', $name, $type));
        }
     ...
     */
    public function clearCache($type = null, $name = null)
    {

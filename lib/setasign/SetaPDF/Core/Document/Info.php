 ...
 * @version    $Id: Info.php 1578 2020-10-18 20:19:10Z jan.slabon $
 */
...
//        $this->_metadata = null;
    ...
    public function getDictionary($create = false)
    {
        ...
        $info = $trailer->getValue('Info');
        if ($info === null || !($info->ensure() instanceof SetaPDF_Core_Type_Dictionary)) {
            if ($create === false) {
                return null;
            }

            $info = $this->getDocument()->createNewObject(new SetaPDF_Core_Type_Dictionary());
            $trailer->offsetSet('Info', $info);
        }

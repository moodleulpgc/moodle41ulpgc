 ...
 * @version    $Id: Image.php 1560 2020-09-24 12:17:20Z maximilian.kresse $
 */
...
abstract class SetaPDF_Core_Image
{
    ...
    static public function getType(SetaPDF_Core_Reader_ReaderInterface $reader)
    {
        ...
        if (strlen($bytes) > 2 && $bytes[0] === "\xFF" && $bytes[1] === "\xD8") {
            $type = self::TYPE_JPEG;
        } elseif (strpos($bytes, "\x89" . 'PNG' . "\x0D\x0A\x1A\x0A") === 0) {
            $type = self::TYPE_PNG;
        } elseif (
            strlen($bytes) > 5 && strpos($bytes, 'GIF') === 0
            && (
                substr($bytes, 3, 3) === '87a' || substr($bytes, 3, 3) === '89a'
            )
        ) {
            $type = self::TYPE_GIF;
        } elseif ($bytes === "\x00\x00\x00\x0C\x6A\x50\x20\x20\x0D\x0A\x87\x0A") {

 ...
 * @version    $Id: Tree.php 1562 2020-09-24 15:47:29Z maximilian.kresse $
 */
    ...
    implements SetaPDF_Core_DataStructure_DataStructureInterface
{
     ...
     * @var integer
     */
    static public $leafNodesPerNode = 64;
    ...
    public function arrange()
    {
        ...
        if ($tmpCount !== 0) {
            ...
            $currentKeys = $currentLeafNode->getValue($entriesKeyName)->ensure();
            ...
            while (count($leafNodes) > self::$leafNodesPerNode) {
                ...
                $currentLeafNode = $leafNodes[$i]['object']->ensure();
                $currentKids = $currentLeafNode->getValue('Kids')->ensure();
                $limits = $currentLeafNode->getValue('Limits');
                $leftLimitsSet = false;

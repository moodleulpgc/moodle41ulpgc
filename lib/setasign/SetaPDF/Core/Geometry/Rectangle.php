 ...
 * @version    $Id: Rectangle.php 1560 2020-09-24 12:17:20Z maximilian.kresse $
 */
...
class SetaPDF_Core_Geometry_Rectangle implements SetaPDF_Core_Geometry_Collidable
{
    ...
    public function __construct($a, $b, $c = null, $d = null)
    {
        if ($a instanceof SetaPDF_Core_Geometry_Point && $b instanceof SetaPDF_Core_Geometry_Point) {
            ...
            $x2 = $point2->getX();
            $y2 = $point2->getY();
        } elseif ($a instanceof SetaPDF_Core_Geometry_Point) {
            $ll = $a;
            $width = $b;
            $height = $c;
    ...
    public function contains($geometry)
    {
        ...
        } elseif ($geometry instanceof SetaPDF_Core_Geometry_Rectangle) {
            $result = (
                $this->_pointInside($geometry->_llX, $geometry->_llY)
                && $this->_pointInside($geometry->_urX, $geometry->_urY)
            );
        } else {
            throw new InvalidArgumentException('Invalid param');
        }
    ...
    public function intersect($geometry)
    {
        if ($geometry instanceof SetaPDF_Core_Geometry_Rectangle) {
            return (
                $this->_llX < $geometry->_urX
                && $this->_urX > $geometry->_llX
                && $this->_llY < $geometry->_urY
                && $this->_urY > $geometry->_llY
            );
        }
    ...
    public function collides(SetaPDF_Core_Geometry_Collidable $geometry)
    {
        if ($geometry instanceof self) {
            return $this->_llX <= $geometry->_urX &&
                $this->_urX >= $geometry->_llX &&
                $this->_llY <= $geometry->_urY &&
                $this->_urY >= $geometry->_llY;
        }

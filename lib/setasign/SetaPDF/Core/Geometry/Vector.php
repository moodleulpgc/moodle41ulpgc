 ...
 * @version    $Id: Vector.php 1560 2020-09-24 12:17:20Z maximilian.kresse $
 */
...
class SetaPDF_Core_Geometry_Vector
{
    ...
    public function multiply($with)
    {
        if ($with instanceof SetaPDF_Core_Geometry_Matrix) {
            list($a, $b,, $c, $d,, $e, $f,) = $with->getValues();
            $x = $a * $this->_x + $c * $this->_y + $e;
            $y = $b * $this->_x + $d * $this->_y + $f;
            $z = $this->_z;
        } else {
            $with = (float)$with;
            $x = $this->_x * $with;

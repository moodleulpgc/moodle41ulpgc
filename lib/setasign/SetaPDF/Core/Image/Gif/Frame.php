...
class SetaPDF_Core_Image_Gif_Frame
{
    ...
    private function _readStream(&$stream, &$transparencyMask, &$transparencyPixelCount)
    {
        if ($this->_previousFrame !== null) {
            // to prevent too many recursions here we ensure the streams of the previous frames in reverse order
            $previousFrames = [];
            ...
            /**
             * @var SetaPDF_Core_Image_Gif_Frame $frame
             */
            foreach (array_reverse($previousFrames) as $frame) {
                $frame->_ensureStream($stream, $transparencyMask, $transparencyPixelCount);
            }
        }

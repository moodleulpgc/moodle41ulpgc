 ...
 * @version    $Id: Callback.php 1475 2020-06-03 10:40:29Z jan.slabon $
 */
...
class SetaPDF_Core_Type_Callback extends SetaPDF_Core_Type_AbstractType
{
    ...
    public function __call($method, array $arguments)
    {
        if (method_exists($this->_value, $method)) {
            if (isset($this->_callbacks['before_' . $method])) {
                call_user_func_array($this->_callbacks['before_' . $method], [$this->_value, &$arguments, $method]);
            }

            $return = call_user_func_array([$this->_value, $method], $arguments);

            if (isset($this->_callbacks['after_' . $method])) {
                call_user_func_array($this->_callbacks['after_' . $method], [$this->_value, &$arguments, &$return, $method]);
            }
    ...
    {
        return $this->__call('toPdfString', [$pdfDocument]);
    }
    ...
    {
        $this->__call('writeTo', [$pdfDocument]);
    }

 ...
 * @version    $Id: AbstractHandler.php 1556 2020-09-22 06:51:24Z jan.slabon $
 */
...
abstract class SetaPDF_Core_SecHandler_AbstractHandler
{
     ...
     * @var array
     */
    protected $_stringAlgorithm = [SetaPDF_Core_SecHandler::ARCFOUR, 5];
     ...
     * @var array
     */
    protected $_streamAlgorithm = [SetaPDF_Core_SecHandler::ARCFOUR, 5];
    ...
    )
    {
        ...
        if ($this->_encryptionDictionary->offsetExists('CF')) {
            ...
            switch ($cryptFilterMethod) {
                case 'V2':
                    $this->_streamAlgorithm = [SetaPDF_Core_SecHandler::ARCFOUR, $keyLength];
                    break;
                case 'AESV2':
                    $this->_streamAlgorithm = [SetaPDF_Core_SecHandler::AES_128, $keyLength];
                    break;
                case 'AESV3':
                    $this->_streamAlgorithm = [SetaPDF_Core_SecHandler::AES_256, $keyLength];
                    break;
                default:
            ...
            switch ($cryptFilterMethod) {
                case 'V2':
                    $this->_stringAlgorithm = [SetaPDF_Core_SecHandler::ARCFOUR, $keyLength];
                    break;
                case 'AESV2':
                    $this->_stringAlgorithm = [SetaPDF_Core_SecHandler::AES_128, $keyLength];
                    break;
                case 'AESV3':
                    $this->_stringAlgorithm = [SetaPDF_Core_SecHandler::AES_256, $keyLength];
                    break;
                default:
                    throw new SetaPDF_Core_SecHandler_Exception(
                        'Unsupported Crypt Filter Method: ' . $cryptFilterMethod,
            ...
            $this->_stringAlgorithm = [SetaPDF_Core_SecHandler::ARCFOUR, $this->_keyLength];
        }

        if ($this->_encryptionDictionary->offsetExists('EncryptMetadata')) {
            $this->_encryptMetadata = (boolean)$this->_encryptionDictionary
    ...
    protected function _crypt($data, $algorithm, $param = null, $encrypt = true)
    {
        ...
        if ($algorithm[0] === SetaPDF_Core_SecHandler::AES_256) {
            if ($encrypt === true) {
        ...
        if ($algorithm[0] === SetaPDF_Core_SecHandler::AES_128) {
            $key .= "\x73\x41\x6c\x54";
        }
    ...
    protected function _computeHashR6($data, $inputPassword, $userKey = '')
    {
        ...
        while (true) {
            ...
            } else if (SetaPDF_Core_SecHandler::$engine === 'openssl' && version_compare(PHP_VERSION, '5.4', '>=')) {
                $e = openssl_encrypt(
                    $k1,
                    'AES-128-CBC',
                    substr($k, 0, 16),

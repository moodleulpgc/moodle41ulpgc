 ...
 * @version    $Id: Standard.php 1562 2020-09-24 15:47:29Z maximilian.kresse $
 */
    ...
    implements SetaPDF_Core_SecHandler_SecHandlerInterface
{
     ...
     */
    static public function ensurePasswordEncoding($revision, $password, $encoding)
    {
        if ($revision <= 4) {
            if ($encoding === null) {
                return $password;
            ...
            return SetaPDF_Core_Encoding::convert($password, $encoding, SetaPDF_Core_Encoding::PDF_DOC);
        }
        ...
        if ($revision == 5 || $revision == 6) {
            ...
            if ($encoding !== 'utf-8') {
                $password = SetaPDF_Core_Encoding::convert($password, $encoding, 'utf-8');
            }

            return SetaPDF_Core_SecHandler_SaslPrep::process($password);
        }
    ...
    {
        return (int)$this->_encryptionDictionary->getValue('R')->getValue();
    }
     ...
     * @throws SetaPDF_Exception_NotImplemented
     */
    public function auth($password = null, $encoding = null)
    {
        $password = (string)$password;
        if ($this->authByOwnerPassword($password, $encoding) !== false) {
            return true;
            
        }

        if ($this->authByUserPassword($password, $encoding) !== false) {
            return true;
        }
     ...
     * @throws SetaPDF_Exception_NotImplemented
     */
    public function authByOwnerPassword($password, $encoding = null)
    {
        $password = self::ensurePasswordEncoding($this->getRevision(), $password, $encoding);
        if (($encryptionKey = $this->_authByOwnerPassword($password)) !== false) {
            $this->_auth = true;
     ...
     * @throws SetaPDF_Exception_NotImplemented
     */
    public function authByUserPassword($password, $encoding = null)
    {
        $password = self::ensurePasswordEncoding($this->getRevision(), $password, $encoding);
        if (($encryptionKey = $this->_authByUserPassword($password)) !== false) {
            $this->_auth = true;
            $this->_encryptionKey = $encryptionKey;
    ...
    protected function _authByUserPassword($userPassword = '')
    {
        ...
        if ($revision < 5) {
            ...
            if ($uValue == $originalUValue ||
                ($revision >= 3 && substr($uValue, 0, 16) == substr($originalUValue, 0, 16))
            ) {
            ...
            if ($hash === substr($uValue, 0, 32)) {
                ...
                } else if (SetaPDF_Core_SecHandler::$engine === 'openssl') {
        ...
        if ($revision < 5) {
            ...
            // (exclusive or)
                ...
                for ($i = 19; $i >= 0; $i--) {
                    ...
                    $userPassword = SetaPDF_Core_SecHandler::arcfour(implode('', $tmp), $userPassword);
                }
            }
    ...
    protected function _computeEncryptionKey($password = '')
    {
        ...
        if ($revision === 5 || $revision === 6) {
            return hash(
                'sha256',
                SetaPDF_Core_SecHandler::generateRandomBytes(128)
                . uniqid('sAlT', true) . microtime() . mt_rand()
    ...
    protected function _computeOValue($userPassword, $ownerPassword = '')
    {
        ...
        if ($revision < 5) {
            ...
            if (3 <= $revision) {
                for ($i = 1; $i <= 19; $i++) {
                    ...
                    $s = SetaPDF_Core_SecHandler::arcfour(implode('', $tmp), $s);
                }
            }
        ...
        if ($revision === 5) {
            ...
            //    as the O key.
            $rand = SetaPDF_Core_SecHandler::generateRandomBytes(16);

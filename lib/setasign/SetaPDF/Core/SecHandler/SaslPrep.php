 ...
 * @version    $Id: SecHandler.php 1410 2020-01-30 15:04:21Z jan.slabon $
 */
 ...
 * @license    https://www.setasign.com/ Commercial
 */
class SetaPDF_Core_SecHandler_SaslPrep
{
    /**
    ...
    static public function process($string)
    {
        ...
        if (preg_match("~[" .
            ...
            "\x{FFF9}-\x{FFFD}" .
            // Inappropriate for canonical representation characters, C.7: https://tools.ietf.org/html/rfc3454#appendix-C.7
            "\x{2FF0}-\x{2FFB}" .
            // Change display properties or deprecated characters, C.8: https://tools.ietf.org/html/rfc3454#appendix-C.8
        ...
        if (preg_match('~[' . $rAndAlCat . ']~u', $string)) {
            ...
            if (!preg_match('~^[' . $rAndAlCat . '].*[' . $rAndAlCat . ']$~u', $string)) {
                throw new InvalidArgumentException(
                    'Checking of bidirectional strings failed. A RandALCat character MUST be the first and last character.'
                );
            }
        }

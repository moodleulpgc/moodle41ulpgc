 ...
 * @version    $Id: Form.php 1562 2020-09-24 15:47:29Z maximilian.kresse $
 */
    ...
    SetaPDF_Core_Canvas_ContainerInterface
{
    ...
    {
        /** @var SetaPDF_Core_Type_Stream $stream */
        $stream = $this->_indirectObject->ensure(true);
        return $stream;
    }
    ...
    public function getBBox($asRect = true)
    {
        if (null === $this->_bbox) {
            ...
            $bbox = $dict->getValue('BBox');
            if ($bbox instanceof SetaPDF_Core_Type_IndirectReference) {
                $bbox = $bbox->ensure();
            }

            if (!$bbox instanceof SetaPDF_Core_Type_Array) {
                throw new SetaPDF_Core_Type_Exception('Required /BBox value is missing or not an array.');
            }
            ...
            $this->_bbox = new SetaPDF_Core_DataStructure_Rectangle($bbox);
        }
    ...
    public function setMatrix($matrix)
    {
        ...
        $dict['Matrix'] = new SetaPDF_Core_Type_Array([
    ...
    public function getGroup()
    {
        ...
        return new SetaPDF_Core_TransparencyGroup($dict->getValue('Group')->ensure());
    }

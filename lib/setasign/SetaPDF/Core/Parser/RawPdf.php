 ...
 * @version    $Id: RawPdf.php 1555 2020-09-21 15:17:20Z maximilian.kresse $
 */

/**
 * A PDF parser for standard tokens.
 *
 * This class doesn't work with final object instances but only returns simple {@link SetaPDF_Core_Type_Raw} instances
...
class SetaPDF_Core_Parser_RawPdf
{
    ...
    private function _readValue($token)
    {
        ...
        // if there are several minus signs remove all but one to have a consistence behavior with Acrobat
    ...
    public function convertToObject(SetaPDF_Core_Type_Raw $data)
    {
        switch ($data->type) {
            ...
            case self::TYPE_ARRAY:
                $entries = array_map([$this, 'convertToObject'], $data->value);
        ...
        throw new InvalidArgumentException('Raw type object has unknown type.');
    }
}
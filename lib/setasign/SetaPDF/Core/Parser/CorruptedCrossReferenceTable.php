 ...
 * @version    $Id: CorruptedCrossReferenceTable.php 1577 2020-10-13 08:30:52Z maximilian.kresse $
 */
    ...
    implements SetaPDF_Core_Parser_CrossReferenceTable_CrossReferenceTableInterface
{
    ...
    protected function _extractObjectIds($buffer, $start)
    {
        ...
        preg_match_all(
            '/(\d+)[' . $delimiters . ']+(\d+)[' . $delimiters . ']+obj/U',
            $buffer,
            $match,
            PREG_OFFSET_CAPTURE
        );
        ...
        if (count($match) > 0) {
            foreach ($match[0] as $key => $data) {
                ...
                if (!isset($this->_parserObjectOffsets[$objId])) {
                    $this->_parserObjectOffsets[$objId] = [];
                }
                ...
                $this->_parserObjectOffsets[$objId][$generation] = [
    ...
    protected function _matchKeywords($buffer, $start)
    {
        foreach (array_keys($this->_matchedPositions) AS $searchFor) {
            if ($searchFor[0] === '~') {
                if (preg_match_all($searchFor, $buffer, $match, PREG_OFFSET_CAPTURE) === 0) {
                    continue;
                }

                $pos = $match[0][count($match[0]) - 1][1];

            } else if (($pos = strrpos($buffer, $searchFor)) === false) {
                continue;
            }

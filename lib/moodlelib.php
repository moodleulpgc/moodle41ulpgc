 ...
 * @return string
 */
function fullname($user, $override=false, $format='') { // ecastro ULPGC to allow naming by lastname, firstname
    ...
    // ecastro ULPGC fullname format
    if (!empty($SESSION->nameformat)) {
        if($SESSION->nameformat == 'firstname') { // ecastro ULPGC get from $SESSION, thus allowing calls as fulname($user), without 3rd parameter
            $template = 'firstname lastname';
        } elseif($SESSION->nameformat == 'lastname') {
            $template = 'lastname, firstname';
        } else {
            $template = 'firstname lastname';
        }
    }
    if (!empty($PAGE->nameformat)) {
        if($PAGE->nameformat == 'firstname') { // ecastro ULPGC get from $PAGE, thus allowing calls as fulname($user), without 3rd parameter
            $template = 'firstname lastname';
        } elseif($PAGE->nameformat == 'lastname') {
            $template = 'lastname, firstname';
        } else {
            $template = 'firstname lastname';
        }
    ...
    if ($override) {
        if (empty($CFG->alternativefullnameformat) || $CFG->alternativefullnameformat == 'language') {
            ...
            if(empty($SESSION->nameformat)) { // ecastro ULPGC apply only if there is something to apply
            return get_string('fullnamedisplay', null, $user);
            }
        } else {
...
function current_language() {
    ...
    } else if (!empty($PAGE->forcedlang)) { // ecastro ULPGC
        $return = $PAGE->forcedlang;

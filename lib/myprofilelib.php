...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if($userdetails = get_config('local_ulpgccore', 'showuserdetails')) { // ecastro ULPGC add detailed profile fields
        $personalcategory = new core_user\output\myprofile\category('private', get_string('privatedetails', 'local_ulpgccore'), null, 'privatedetails');
        $tree->add_category($personalcategory);
        $private = 'private';
    } else {
        $private = 'contact';
    }
    ...
    // Contact details.
    if (has_capability('moodle/user:viewhiddendetails', $courseorusercontext) || $iscurrentuser) { // ecastro ULPGC allow user to see all his own details
        $hiddenfields = array();
    } else {
        $hiddenfields = array_flip(explode(',', $CFG->hiddenuserfields));
    }

    $canviewuseridentity = (has_capability('moodle/site:viewuseridentity', $courseorusercontext) || $iscurrentuser); // ecastro ULPGC allow user to see all his own details
    if ($canviewuseridentity) {
        ...
        if($userdetails = get_config('local_ulpgccore', 'showuserdetails')) { // ecastro ULPGC add detailed profile fields
            $identityfields = $identityfields + array_flip(explode(',', $userdetails));
        }
    } else {
        $identityfields['email'] = 1; // ecastro ULPGC allow users to see e-mails when not in lists
    }
    ...
    if (isset($identityfields['phone1']) && $user->phone1) {
        $node = new core_user\output\myprofile\node($private, 'phone1', get_string('phone1'), null, null, $user->phone1);
        $tree->add_node($node); // ecastro ULPGC private
    }

    if (isset($identityfields['phone2']) && $user->phone2) {
        $node = new core_user\output\myprofile\node($private, 'phone2', get_string('phone2'), null, null, $user->phone2);
        $tree->add_node($node); // ecastro ULPGC private
    }
    ...
    if (isset($identityfields['department']) && $user->department) {
        $node = new core_user\output\myprofile\node($private, 'department', get_string('department'), null, null,
            $user->department);
        $tree->add_node($node);
    }
    ...
    if (!isset($hiddenfields['firstaccess']) && !empty($course)) { // ecastro ULPGC !course: allow access to info for teachers"
        if ($user->firstaccess) {

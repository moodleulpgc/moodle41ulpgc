...
if(get_config('local_ulpgccore', 'enabledadvancedgrades')) {
    require_once($CFG->dirroot . '/local/ulpgccore/gradelib.php');
}
...
class grade_category extends grade_object {
    ...
    public function generate_grades($userid=null, ?\core\progress\base $progress = null) {
        ...
        if (empty($depends_on)) {
            ...
            $sql = "SELECT *
                      FROM {grade_items}
                     WHERE id $usql
                     ORDER BY sortorder ASC "; // ecastro ULPGC to be able to apply order dependent aggregation rules
        ...
        foreach ($items as $itemid=>$value) {
            if (!isset($grade_values[$itemid]) and !in_array($itemid, $excluded)) {
                if (!$this->aggregateonlygraded) {
                    ...
                    $nullgrades[$itemid] = 1; // ecastro ULPGC to keep track of empty items in aggregations
                } else {
    ...
    public function aggregate_values_and_adjust_bounds($grade_values,
                                                       ...
                                                       $grademinoverrides = array(),
                                                       $grademaxoverrides = array(),
                                                       $novalue = array()) { // ecastro ULPGC custom aggregations
        global $CFG;
        ...
        if(get_config('local_ulpgccore', 'enabledadvancedgrades')) {
            if(in_array($this->aggregation, explode(',', GRADE_ULPGC_AGGREGATIONS))) { // ecastro ULPGC put in use custom ULPGC grade aggregations
                return local_ulpgccore_aggregate_values_bounds($this, $grade_values, $items, $weights,
                                                                       $grademinoverrides, $grademaxoverrides, $novalue);
            }
        }

...
/**
 * Unit tests for the response condition.
 *
...
use availability_response\condition;
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class availability_response_condition_testcase extends advanced_testcase {
    /**
    ...
    /**
     * Tests constructing and using response condition as part of tree.
     */
    ...
    public function test_constructor() {
        ...
        try {
            $response = new condition($structure);
            $this->fail();
        ...
        $response = new condition($structure);
        $this->assertEquals('{response:>= 2014-02-18 14:43:17}', (string)$response);
        $structure->d = '<';
        $response = new condition($structure);
        $this->assertEquals('{response:< 2014-02-18 14:43:17}', (string)$response);
    }
    ...
    public function test_is_available() {
        ...
        // Test with >=.
        $response = new condition((object)array('d' => '>=', 't' => $time));
        condition::set_current_time_for_test($time - 1);
        $this->assertFalse($response->is_available(false, $info, true, $USER->id));
        condition::set_current_time_for_test($time);
        $this->assertTrue($response->is_available(false, $info, true, $USER->id));
        ...
        $response = new condition((object)array('d' => '<', 't' => $time));
        condition::set_current_time_for_test($time);
        $this->assertFalse($response->is_available_for_all(false));
        condition::set_current_time_for_test($time - 1);
        $this->assertTrue($response->is_available_for_all(false));
    }
    ...
    public function test_get_description() {
        ...
        // Test with >=.
        $response = new condition((object)array('d' => '>=', 't' => $time));
        $information = $response->get_description(true, false, $info);
        $this->assertRegExp('~after.*18 February 2014, 2:55 (PM|pm)~', $information);
        $information = $response->get_description(true, true, $info);
        $this->assertRegExp('~before.*18 February 2014, 2:55 (PM|pm)~', $information);
        $information = $response->get_standalone_description(true, false, $info);
        $this->assertRegExp('~from.*18 February 2014, 2:55 (PM|pm)~', $information);
        $information = $response->get_standalone_description(true, true, $info);
        ...
        // Test special case for responses that are midnight.
        $response = new condition((object)array('d' => '>=',
        ...
        // if the response is 5 March 00:00, then we show it as available until 4

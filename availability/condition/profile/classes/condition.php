...
class condition extends \core_availability\condition {
    ...
    public static function get_standard_profile_fields(): array {
        return [
            ...
            // ecastro ULPGC additional fields
            'picture' => \core_user\fields::get_display_name('pictureofuser'),
            'timezone' => \core_user\fields::get_display_name('timezone'),
            'policyagreed' => \core_user\fields::get_display_name('policyagreement'),
            // ecastro ULPGC
        ];
    }
        ...
        if ($iscurrentuser) {
            if (!$iscustomprofilefield) {
                if (property_exists($USER, $field)) {
                    ...
                    mtrace('Requested user profile field does not exist: '.$field);
                    $value = $DB->get_field('user', $field, array('id'=>$USER->id)); // ecastro treat error in forum_cron 
                    if($value !== false) {
                        $USER->{$field} = $value; // ecastro needed for extra fields in availability 
                        return $value;
                    }

...
/**
 * Unit tests for the timeelapsed condition.
 *
...
use availability_timeelapsed\condition;
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class availability_timeelapsed_condition_testcase extends advanced_testcase {
    /**
    ...
    /**
     * Tests constructing and using timeelapsed condition as part of tree.
     */
    ...
    public function test_constructor() {
        ...
        try {
            $timeelapsed = new condition($structure);
            $this->fail();
        ...
        $timeelapsed = new condition($structure);
        $this->assertEquals('{timeelapsed:>= 2014-02-18 14:43:17}', (string)$timeelapsed);
        $structure->d = '<';
        $timeelapsed = new condition($structure);
        $this->assertEquals('{timeelapsed:< 2014-02-18 14:43:17}', (string)$timeelapsed);
    }
    ...
    public function test_is_available() {
        ...
        // Test with >=.
        $timeelapsed = new condition((object)array('d' => '>=', 't' => $time));
        condition::set_current_time_for_test($time - 1);
        $this->assertFalse($timeelapsed->is_available(false, $info, true, $USER->id));
        condition::set_current_time_for_test($time);
        $this->assertTrue($timeelapsed->is_available(false, $info, true, $USER->id));
        ...
        $timeelapsed = new condition((object)array('d' => '<', 't' => $time));
        condition::set_current_time_for_test($time);
        $this->assertFalse($timeelapsed->is_available_for_all(false));
        condition::set_current_time_for_test($time - 1);
        $this->assertTrue($timeelapsed->is_available_for_all(false));
    }
    ...
    public function test_get_description() {
        ...
        // Test with >=.
        $timeelapsed = new condition((object)array('d' => '>=', 't' => $time));
        $information = $timeelapsed->get_description(true, false, $info);
        $this->assertRegExp('~after.*18 February 2014, 2:55 (PM|pm)~', $information);
        $information = $timeelapsed->get_description(true, true, $info);
        $this->assertRegExp('~before.*18 February 2014, 2:55 (PM|pm)~', $information);
        $information = $timeelapsed->get_standalone_description(true, false, $info);
        $this->assertRegExp('~from.*18 February 2014, 2:55 (PM|pm)~', $information);
        $information = $timeelapsed->get_standalone_description(true, true, $info);
        ...
        // Test special case for timeelapseds that are midnight.
        $timeelapsed = new condition((object)array('d' => '>=',
        ...
        // if the timeelapsed is 5 March 00:00, then we show it as available until 4

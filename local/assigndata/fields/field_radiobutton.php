...
/**
 * Plugin local_assigndata class radiobutton
 *
...
global $CFG;
require_once($CFG->dirroot.'/mod/data/field/radiobutton/field.class.php');

/**
 * The field radiobutton class
 *
 * @package    local_assigndata
 * @copyright  2017 Enrique Castro @ ULPGC
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class field_radiobutton extends \data_field_radiobutton {
...
use field_base {
    ...
    function add_submission_form_elements($mform, $content = '') {
        ...
        foreach($options as $idx => $radio) {
            $grouparr[] = $mform->createElement('radio', 'radiob', '', $radio, $radio);
        }
        $mform->addGroup($grouparr, $prefix.'content', $fieldname, array(' '));
        $mform->setDefault($prefix.'content[radiob]', $content->content);
        $mform->setType($prefix.'content[radiob]', PARAM_TEXT);

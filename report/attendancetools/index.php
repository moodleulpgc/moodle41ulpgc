...
print_object($pageparams);
print_object($att->pageparams);

if($att->pageparams->action ==  99) {
    $now = time();
    ...
    if(!$groupid) {
        ...
        $formdata->sessiontype = mod_attendance_structure::SESSION_GROUP;
    }
    ...
    redirect($url);    
            ...
            mod_attendance_notifyqueue::notify_problem(get_string('noaction', 'report_attendancetools'));
...
switch ($att->pageparams->action) {
    ...
    case mod_attendance_sessions_page_params::ACTION_DELETE:
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('attendanceforthecourse', 'attendance').' :: ' .format_string($course->fullname)); // ecastro ULPGC

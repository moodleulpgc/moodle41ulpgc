 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class media_bustreaming_testcase extends advanced_testcase {
    ...
    public function setUp() {
        ...
        \core\plugininfo\media::set_enabled_plugins('bustreaming');
    ...
    public function test_is_installed() {
        ...
        $this->assertEquals(['bustreaming' => 'bustreaming'], $sortorder);
    }
    ...
    public function test_supported() {
        ...
        // Format: bustreaming.
        $url = new moodle_url('http://www.bustreaming.com/watch?v=vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $url = new moodle_url('http://www.bustreaming.com/v/vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $url = new moodle_url('http://m.bustreaming.com/watch?v=vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);

        // Format: bustreaming video within playlist.
        $url = new moodle_url('https://www.bustreaming.com/watch?v=dv2f_xfmbD8&index=4&list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertContains('list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0', $t);

        // Format: bustreaming video with start time.
        $url = new moodle_url('https://www.bustreaming.com/watch?v=JNJMF1l3udM&t=1h11s');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertContains('start=3611', $t);

        // Format: bustreaming video within playlist with start time.
        $url = new moodle_url('https://www.bustreaming.com/watch?v=dv2f_xfmbD8&index=4&list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0&t=1m5s');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertContains('list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0', $t);
        $this->assertContains('start=65', $t);

        // Format: bustreaming video with invalid parameter values (injection attempts).
        $url = new moodle_url('https://www.bustreaming.com/watch?v=dv2f_xfmbD8&index=4&list=PLxcO_">');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertNotContains('list=PLxcO_', $t); // We shouldn't get a list param as input was invalid.
        $url = new moodle_url('https://www.bustreaming.com/watch?v=JNJMF1l3udM&t=">');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertNotContains('start=', $t); // We shouldn't get a start param as input was invalid.

        // Format: bustreaming playlist.
        $url = new moodle_url('http://www.bustreaming.com/view_play_list?p=PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $url = new moodle_url('http://www.bustreaming.com/playlist?list=PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $url = new moodle_url('http://www.bustreaming.com/p/PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
    ...
    public function test_embed_url() {
        ...
        $this->assertRegExp('~mediaplugin_bustreaming~', $content);

 ...
 */
 ...
 if($ulpgcgroups = get_config('local_ulpgcgroups')) { 
    require_once $CFG->dirroot.'/local/ulpgcgroups/lib.php'; // ecastro ULPGC
 }
 ...
 * @param int $maxgroups number of groups created, determine the lenght oh the number label
 * @return string the parsed format string
 */
function groups_parse_name($format, $groupnumber, $maxgroups=1) { // ULPGC ecastro
    if (strstr($format, '@') !== false) { // Convert $groupnumber to a character series
        ...
        $n = strlen(strval($maxgroups));
        $num = sprintf("%0{$n}u", $groupnumber+1); // ULPGC ecastro to better group number printing
        $str = str_replace('#', $num, $format);
    }
    return($str);
}

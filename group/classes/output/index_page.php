...
/* custom gropup ecastro ULPGC  */

    /** @var bool Whether to use custom ULPGC group settings or not. */
    public $ulpgcgroups;

    /** @var int $groupingid The grouping ID. */
    public $groupingid;
    
    /** @var int $userorder How to sort users in lists. */
    public $userorder;
    ...
    public function __construct($courseid, $groups, $selectedgroupname, $selectedgroupmembers, $disableaddedit, $disabledelete,
                                $undeletablegroups, $ulpgcgroups=false, $groupingid=0, $userorder=1) {
        ...
        // custom group fields ecastro ULPGC
        $this->ulpgcgroups = $ulpgcgroups;
    ...
    public function export_for_template(renderer_base $output) {
        ...
        // custom group fields ecastro ULPGC
        $data->ulpgcgroups = $this->ulpgcgroups;
        $data->groupingid = $this->groupingid;
        $data->userorder = $this->userorder;
        $data->removemembersdisabled = ($this->ulpgcgroups AND $this->disableaddedit) ? empty($data->members) : true;

                ...
                if (selectEl && o.responseText) {
                    ...
                    for (var i=0; i<roles.length; i++) {
                        ...
                        for(var j=0; j<roles[i].users.length; j++) {
                            ...
                            optionEl.setAttribute("style", roles[i].users[j].style); // ecastro ULPGC to style controlled group membership
...
UpdatableMembersCombo.prototype.refreshMembers = function () {
    ...
    var order=0;
    var selectORDER = document.getElementsByName("order")[0];  // ecastro ULPGC, to enforce naming format
    if( selectORDER ) {
    ...
    if(document.getElementById('emptygroup') != null) { // ecastro ULPGC improve groups interface
        document.getElementById("emptygroup").disabled = !singleSelection; 
    }
    if(document.getElementById('removeuser') != null) { // ecastro ULPGC improve groups interface
        document.getElementById("removeuser").disabled = !singleSelection; 
    }


    if(singleSelection) {
        var sUrl = this.wwwRoot+"/group/index.php?id="+this.courseId+"&group="+groupId+"&order="+order+"&act_ajax_getmembersingroup"; // ecastro ULPGC put order param in use
        var self = this;

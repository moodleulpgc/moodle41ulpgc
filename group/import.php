...
if ($importform->is_cancelled()) {
    ...
    if($ulpgcgroups = get_config('local_ulpgcgroups', 'enabledadvancedgroups')) {  // ecastro ULPGC customize groups page  // Kirill Astashov
    ...
    foreach ($header as $i => $h) {
        ...
        if (!(isset($required[$h]) or isset($optionalDefaults[$h]) or isset($optional[$h]) or $h=='member')) { // ecastro ULPGC
    ...
    while ($line = $csvimport->next()) {
        ...
        $record = array(); // to avoid carry on from line to next line (e.g. avoids adding a user in one line to all groups created)
        ...
        if (trim(implode($line)) !== '') {
            ...
            foreach ($record as $name => $value) {
                $value = trim($value,' "\''); // remove whitespace and quotes // ecastro ULPGC // Kirill Astashov
            ...
            if (isset($newgroup->courseid)) {
                ...
                if (!has_capability('moodle/course:managegroups', $newgrpcoursecontext) or (!is_enrolled($newgrpcoursecontext) and !has_capability('moodle/course:view', $newgrpcoursecontext))) {
                    ...
                    if ($groupid = groups_get_group_by_name($newgroup->courseid, $groupname)) {
                        ...
                        $adduser = true; // ecastro ULPGC // Kirill Astashov
                        ...
                        $adduser = false; // ecastro ULPGC // Kirill Astashov
                        continue;
                    }

                    // if there are user data, add user group membership // ecastro ulpgc
                    if($message = local_ulpgcgroups_import_group_users($adduser, $newgroup, $context)) {
                        echo $message;
                    }

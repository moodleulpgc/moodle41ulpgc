...
if($ulpgcgroups = get_config('local_ulpgcgroups')) { // ecastro ULPGC 
    $groupingid = optional_param('grouping', 0, PARAM_INT); 
    $userorder = optional_param('order', 1, PARAM_INT); 
    //$returnurl = new moodle_url('index.php', array('id'=>$courseid, 'order'=>$userorder, 'grouping'=>$groupingid));
    $returnurl = new moodle_url($returnurl, array('order'=>$userorder, 'grouping'=>$groupingid));
}
...
if ($confirm && data_submitted()) {
    ...
    $ulpgcgroups = get_config('local_ulpgcgroups', 'enabledadvancedgroups');
    foreach($groupidarray as $groupid) {
        if(!$ulpgcgroups || local_ulpgcgroups_delete_group_allowed($groupid)) { // ecastro ULPGC enforce non deleting component-created groups
            groups_delete_group($groupid);
        } else {
            echo $OUTPUT->notification(get_string('deletenotallowed', 'local_ulpgcgroups'));
        }
    }
    ...
    $PAGE->navbar->add(get_string('deleteselectedgroup', 'group'));

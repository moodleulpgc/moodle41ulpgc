...
class grouping_form extends moodleform {
    ...
    function definition () {
        ...
        if(get_config('local_ulpgcgroups')) { // ecastro ULPGC to allow exclusive groupings in configdata
            $mform->addElement('hidden', 'configdata');
...
function definition_after_data(){ // ecastro ULPGC to allow exclusive groupings  in configdata
    $mform = $this->_form;
    if($mform->elementExists('configdata')) {
        if($element = $mform->getElement('configdata')) {
            $current = $element->getValue();
            if(!$configdata = json_decode($current)) {
                $configdata = new stdClass();
                $configdata->exclusive = 0;
            }
        }
        $current = json_encode($configdata);
        $configdata->exclusive = 0;
        $value0 = json_encode($configdata);
        $configdata->exclusive = 1;
        $value1 = json_encode($configdata);
        $mform->removeElement('configdata');
        
        $element = $mform->createElement('advcheckbox', 'configdata', get_string('singlegroupmembership', 'local_ulpgcgroups'),
                                get_string('explainsinglegroupmembership', 'local_ulpgcgroups'), array(), array($value0, $value1));
        $mform->insertElementBefore($element, 'description_editor');  
        $mform->addHelpButton('configdata', 'singlegroupmembership', 'local_ulpgcgroups');
        $mform->setType('configdata', PARAM_RAW);
        $mform->setDefault('configdata', $current);

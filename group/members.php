...
if($ulpgcgroups = get_config('local_ulpgcgroups', 'enabledadvancedgroups')) {  // ecastro ULPGC customize groups page
    ...
    $sourcegroup = optional_param('source', 0, PARAM_INT);  
    $exclusive = optional_param('exclusive', 0, PARAM_INT); 

    $ulpgcparams = array('grouping'=>$groupingid, 'order'=>$userorder, 'source'=>$sourcegroup, 'exclusive'=>$exclusive);
    ...
    $thisurl = new moodle_url('/group/members.php', array('id'=>$course->id, 'group'=>$group->id)  + $ulpgcparams); // ecastro ULPGC
} else {
    $ulpgcparams = array();
}


$PAGE->set_url('/group/members.php', array('group'=>$groupid) + $ulpgcparams);
...
$returnurl = new moodle_url('/group/index.php', array('id'=>$course->id, 'group'=>$group->id)  + $ulpgcparams); // ecastro ULPGC

if ($cancel) {
    redirect($returnurl);
}

$groupmembersselector = new group_members_selector('removeselect', array('groupid' => $groupid, 'courseid' => $course->id, 'single'=>$exclusive)); // ecastro ULPGC
$potentialmembersselector = new group_non_members_selector('addselect', array('groupid' => $groupid, 'courseid' => $course->id, 'source' =>$sourcegroup, 'single'=>$exclusive)); // ecastro ULPGC
...
if($ulpgcgroups) {
    ...
    echo local_ulpgcgroups_controlledalert($group);
    echo local_ulpgcgroups_sourcegroup_menu($thisurl);
    echo local_ulpgcgroups_exclusivegrouping_menu($thisurl);
    echo local_ulpgcgroups_nameformat_menu($thisurl);
}
...
<div id="addmembersform">
    ...
    <?php if($ulpgcgroups) {  // ecastro ULPGC to enforce source group & extra settings ?>
        <input type="hidden" name="order" value="<?php p($userorder); ?>" />
        <input type="hidden" name="source" value="<?php p($sourcegroup); ?>" />
        <input type="hidden" name="exclusive" value="<?php p($exclusive); ?>" />

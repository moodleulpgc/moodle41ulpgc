...
if(get_config('local_ulpgcgroups', 'enabledadvancedgroups')) { // ecastro ULPGC adding custom params
...
if ($id and $delete) {
    ...
    if (!$confirm) {
        ...
        $PAGE->navbar->add($strgroupings, new moodle_url('/group/groupings.php', array('id'=>$courseid))); // ecastro ULPGC better navigation
        $PAGE->navbar->add(get_string('deletegrouping', 'group'));
        echo $OUTPUT->header();
        $optionsyes = array('id'=>$id, 'delete'=>1, 'courseid'=>$courseid, 'sesskey'=>sesskey(), 'confirm'=>1);
        $optionsno  = array('id'=>$courseid);
        $formcontinue = new single_button(new moodle_url('grouping.php', $optionsyes + $ulpgcparams), get_string('yes'), 'get');
        $formcancel = new single_button(new moodle_url('groupings.php', $optionsno + $ulpgcparams), get_string('no'), 'get');

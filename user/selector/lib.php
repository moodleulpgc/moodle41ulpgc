...
abstract class user_selector_base {
    ...
    protected function output_optgroup($groupname, $users, $select) {
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ...
                if(isset($this->restrictedgroups) && $this->restrictedgroups) { // ecastro ULPGC enforce component restricted groups
                    if(isset($user->component) && $user->component != '') { 
                        $attributes .= ' style="color:maroon;" disabled="disabled" ';
                    }
                }
                $doubled = (isset($user->nonsingle) && $user->nonsingle) ? '* ' : '';

                unset($this->selected[$user->id]);
                $output .= '    <option' . $attributes . ' value="' . $user->id . '">' .
                        $doubled.$this->output_user($user) . "</option>\n";
    ...
    public function output_user($user) {
        ...
        // ecastro ULPGC above changes to avoid empty fields
        return $out;
    }
...
abstract class groups_user_selector_base extends user_selector_base {
    ...
    public function __construct($name, $options) {
        ...
        $this->restrictedgroups = get_config('local_ulpgcgroups', 'forcerestrictedgroups'); // ecastro ULPGC enforce component restricted groups
        if($ulpgcgroups = get_config('local_ulpgcgroups', 'enabledadvancedgroups')) { // ecastro ULPGC decoration for improved groups management
            $fields = array('source', 'single');
            foreach($fields as $field) {
                $this->{$field} = '';
    ...
    protected function get_options() {
        ...
        $options['courseid'] = $this->courseid;
            $options['source']      = isset($this->source) ? $this->source : 0; // ecastro ULPGC
            $options['single']      = isset($this->single) ? $this->single : 0; // ecastro ULPGC
        return $options;
    }
...
class group_members_selector extends groups_user_selector_base {
    ...
    public function find_users($search) {
        ...
        if($ulpgcgroups = get_config('local_ulpgcgroups', 'enabledadvancedgroups')) { // ecastro ULPGC decoration for improved groups management
            ...
            $roles = local_ulpgcgroups_group_selector_roles($roles, $this->groupid, $this->single);
        }
...
class group_non_members_selector extends groups_user_selector_base {
    ...
    public function find_users($search) {
        ...
        if($ulpgcgroups = get_config('local_ulpgcgroups', 'enabledadvancedgroups')) {         // ecastro ULPGC additional searching for improved groups management
            global $CFG;
            include_once($CFG->dirroot.'/local/ulpgcgroups/lib.php');
            list($groupsconditions, $groupsparams) = local_ulpgcgroups_nongroup_selector($this->groupid, $this->source, $this->single);
            $searchcondition .= $groupsconditions;
            $searchparams = $searchparams + $groupsparams;
        }

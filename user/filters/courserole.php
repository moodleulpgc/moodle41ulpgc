...
class user_filter_courserole extends user_filter_type {
    ...
    public function setupForm(&$mform) {
        ...
        $objs['rnot'] =& $mform->createElement('checkbox', $this->_name.'_not', null, get_string('notrole', 'local_ulpgccore')); // ULPGC ecastro
        ...
        $objs['clist'] =& $mform->createElement('checkbox', $this->_name.'_list', null, get_string('courselist', 'local_ulpgccore'));  // ULPGC ecastro
    ...
    public function check_data($formdata) {
        ...
        $role     = $field .'_rl';
        $category = $field .'_ct';
        $notrole  = $field.'_not'; // ecastro ULPGC
        $clist    = $field .'_list';
        ...
        if (property_exists($formdata, $field)) {
            ...
            return array('value'      => (string)$formdata->$field,
                         ...
                         'not'        => (int)isset($formdata->$notrole),
                         'list'       => (int)isset($formdata->$clist));
        }
        return false;
    }
    ...
    public function get_sql_filter($data) {
        ...
        $notrole    = $data['not']; // ULPGC ecastro
        $clist       = $data['list'];
        ...
        $notclause = ''; // ecastro ULPGC
        if($notrole) {
           $notclause = ' NOT ';
        }
        return array("id $notclause IN (SELECT userid
    ...
    public function get_label($data) {
        ...
        $roleid     = $data['roleid'];
        $categoryid = $data['categoryid'];
        $notrole    = $data['not']; // ecastro ULPGC
        $clist       = $data['list'];
        ...
        $notclause = '';
        if($notrole) {
            $notclause = get_string('notrole', 'local_ulpgccore').'&nbsp;'; // ecastro ULPGC
        }
        if ($roleid) {
            $role = $DB->get_record('role', array('id' => $roleid));
            $a->rolename = '"'.$notclause.role_get_name($role).'"'; // ecastro ULPGC
        } else {
        ...
        $listclause = '';
        if($clist) {
            $listclause = '&nbsp;('.get_string('courselist', 'local_ulpgccore').')';   // ULPGC ecastro
        }
        if ($value) {
            $a->coursename = '"'.s($value).'"'.$listclause;
            if(!$clist) {
            if (!$DB->record_exists('course', array('shortname' => $value))) {
                ...
                $list = filters_list_convert($value, 1);
                if($list) {
                    if (!$DB->record_exists_select('course', " shortname IN ( $list ) ")) {
                        return '<span class="notifyproblem">'.get_string('courserolelabelerror', 'filters', $a).'</span>';
                    }
                }
            }

...
class user_filter_text extends user_filter_type {
    ...
    public function getOperators() {
        return array(0 => get_string('contains', 'filters'),
                     ...
                     6 => get_string('inlist','local_ulpgccore'),
                     7 => get_string('notinlist','local_ulpgccore')); // ULPGC ecastro
    }
    ...
    public function get_sql_filter($data) {
        ...
        switch($operator) {
            ...
            case 5: // Empty.
                ...
                $params[$name] = '';
                break;
            case 6: // in list   // ULPGC ecastro
                if($list) {
                    $res = " $field  IN ( $list ) ";
                } else {
                    $res = $DB->sql_like($field, ":$name", false, false);   //" $field  $ilike '%$value%'";
                }
                $params[$name] = "%$value%";
                break;
            case 7: // not in list ecastro
                if($list) {
                    $res = " $field  NOT IN ( $list ) ";
                } else {
                    $res = $DB->sql_like($field, ":$name'", false, false, true);  //" $field  NOT $ilike '%$value%'";
                }
                $params[$name] = "%$value%";
                break;
            default:

...
if (has_capability('moodle/grade:manage', $systemcontext)
 or has_capability('moodle/grade:manageletters', $systemcontext)) { // speedup for non-admins, add all caps used on this page
    ...
    if ($ADMIN->fulltree) {
        ...
        if($ulpgc = get_config('local_ulpgccore', 'enabledadvancedgrades')) { // ecastro ULPGC grade aggregation types
            include_once($CFG->dirroot.'/local/ulpgccore/gradelib.php');
            $ulpgcaggs = array(GRADE_AGGREGATE_ULPGC_SUM        =>new lang_string('aggregateulpgcsum', 'local_ulpgccore'),
                                GRADE_AGGREGATE_ULPGC_MEAN_ACTV =>new lang_string('aggregateulpgcmeanactv', 'local_ulpgccore'),
                                GRADE_AGGREGATE_ULPGC_MEAN_EXAM =>new lang_string('aggregateulpgcmeanexam', 'local_ulpgccore'),
                                GRADE_AGGREGATE_ULPGC_MEAN_CONVO=>new lang_string('aggregateulpgcmeanconvo', 'local_ulpgccore'),
                                GRADE_AGGREGATE_ULPGC_FINAL     =>new lang_string('aggregateulpgfinal', 'local_ulpgccore'),
                                GRADE_AGGREGATE_ULPGC_NONE     =>new lang_string('aggregateulpgnone', 'local_ulpgccore'),
                         );
            $options = $options + $ulpgcaggs;
        }

...
$roleunassign    = optional_param('roleunassign', 0, PARAM_RAW);
...
if ($accept) {
    ...
    $optionsyes['roleunassign'] = $roleunassign; // ecastro ULPGC
...
if ($confirm) {
    ...
    foreach ($SESSION->bulk_courses as $courseid) {
        ...
        foreach ($SESSION->bulk_users as $userid) {
            // ecastro ULPGC
            if($roleunassign) {
                role_unassign($roleunassign, $userid, $context->id);
            } else {
                role_unassign_all(['userid' => $userid, 'contextid' => $context->id]); // ecastro ULPGC
            }
        }
    }
...
$rolenames = '<option value="0">'.advuserbulk_get_string('allroles', $pluginname).'</option> ' ;
    ...
    <form id="addform" method="post" action="index.php">
        <table cellpadding="6" class="selectcourses generaltable generalbox boxaligncenter" summary="">
          ...
          <td id="potentialcell">
                ...
                <tr>
                    <td align="center">
                        <label for="roleunassign"><?php echo advuserbulk_get_string('roletoremove', $pluginname) ?></label>
                        <br />
                        <select name="roleunassign" id="roleunassign" size="1">
                        <?php echo $rolenames ?>
                                </select>
                    </td>
                </tr>

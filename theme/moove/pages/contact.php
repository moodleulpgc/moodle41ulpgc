...
// Set page URL.
$PAGE->set_url('/theme/moove/pages/contact.php');
...
$PAGE->add_body_class('theme_moove-contact');

// Get theme config.
$config = get_config('theme_moove');

// If the contact page is disabled, we just show a short friendly warning page and are done.
if ($config->enablecontact != THEME_MOOVE_SETTING_SELECT_YES) {
    echo $OUTPUT->header();
    $notification = new \core\output\notification(get_string('contactdisabled', 'theme_moove'),
...
// Set page title.
$PAGE->set_title(theme_moove_get_staticpage_pagetitle('contact'));

// Start page output.
echo $OUTPUT->header();

// Show page heading.
echo $OUTPUT->heading(theme_moove_get_staticpage_pagetitle('contact'));

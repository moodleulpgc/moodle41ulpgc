...
class boostnavbar extends \theme_boost\boostnavbar {
    ...
    protected function prepare_nodes_for_boost(): void {
        ...
        if ($this->page->context->contextlevel == CONTEXT_COURSE) {
            if (get_config('theme_moove', 'categorybreadcrumbs') == THEME_MOOVE_SETTING_SELECT_YES) {
        ...
        if ($this->page->context->contextlevel == CONTEXT_SYSTEM) {
            ...
            // ecastro ULPGC make admin NO breadcrumb
            if($this->page->pagelayout == 'admin') {
                if(($this->page->pagetype == 'admin-search') || (substr($this->page->pagetype, 0, 6) ==  'admin-')) {
                    $this->clear_items();
                } else {

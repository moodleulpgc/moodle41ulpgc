 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class examboard_upload_form extends moodleform {

    /**
     * Returns the options array to use in filemanager for examboard attachments
     *
     * @param stdClass $examboard
     * @return array
     */
    public static function attachment_options($examboard) {
        global $COURSE, $PAGE, $CFG;
        $maxbytes = get_user_max_upload_file_size($PAGE->context, $CFG->maxbytes, $COURSE->maxbytes, $examboard->maxbytes);
        return array(
            'subdirs' => 0,
            'maxbytes' => $maxbytes,
            'maxfiles' => $examboard->maxattachments,
            'accepted_types' => '*',
    ...
    function definition() {
        ...
        $mform->addElement('static', 'student', get_string('filesfor', 'examboard'), fullname($user));
        ...
        /*
            ...
            $mform->addElement('editor', 'uploadlinks', get_string('uploadlinks', 'examboard'), array('rows' => 1.8), $options);
            $mform->setType('uploadlinks', PARAM_RAW);
            $mform->hideIf('uploadlinks', 'action', 'neq', 'upload_user');
        */
        
        $maxfiles = 1;
        if($action == 'upload_examination' || $action == 'upload_board') {

    ...
    public function get_cells($remarks = false) {
        foreach ($this->reportdata->sessions as $sess) {
            ...
            !empty($this->reportdata->sessionslog[$this->user->id][$sess->id]->statusid)) {
                ...
                if ($remarks) {
                    //ecastro ULPGC
                    $seated = $this->reportdata->sessionslog[$this->user->id][$sess->id]->seat;
                    $seat = '';
                    if($seated) {
                        $seat = new \stdClass();
                        $seat->row = strstr($seated, '_',  true);
                        $seat->col = trim(strstr($seated, '_'), '_ -');
                        $seat = get_string('seatlabel', 'attendance', $seat).' - ';
                    }
                    $remark = $this->reportdata->sessionslog[$this->user->id][$sess->id]->remarks;
                    $this->construct_remarks_cell($seat.$remark);
                }
            } else {

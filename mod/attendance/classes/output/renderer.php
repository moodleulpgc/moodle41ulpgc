...
class renderer extends plugin_renderer_base {
    ...
    private function construct_fullname_head($data) {
        ...
        if ($data->pageparams->sort == ATT_SORT_LASTNAME) {
            ...
            $SESSION->nameformat = 'lastname'; // ecastro ULPGC
            ...
            $SESSION->nameformat = 'firstname'; // ecastro ULPGC
        } else {
            ...
            $SESSION->nameformat = ''; // ecastro ULPGC
        }
        ...
        if ($user->enrolmentend && $user->enrolmentend < $takedata->sessioninfo->sessdate) {
            ...
            if($takedata->att->seating) {  
                ...
                if (array_key_exists($user->id, $takedata->sessionlog)) {
                    $params['value'] = $takedata->sessionlog[$user->id]->seat;
                }
                $seat = html_writer::empty_tag('input', $params);
            }
            // ecastro ULPGC
            $params = array(
                    'type'  => 'text',
            ...
            $celldata['text'][] = $seat.html_writer::empty_tag('input', $params); //ecastro ULPGC
    ...
    private function construct_user_sessions_log(user_data $userdata) {
        ...
        if($userdata->filtercontrols->att->seating) {  // ecastro ULPGC
            $table->head[] = get_string('place', 'attendance');
        } else {

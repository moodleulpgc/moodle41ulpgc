...
class mod_attendance_structure {
    ...
    public function update_session_from_form_data($formdata, $sessionid) {
        ...
        // ecastro ULPGC
        if (!empty($formdata->seatrows)) {
            $sess->seatrows = $formdata->seatrows;
        }
        if (!empty($formdata->seatcols)) {
            $sess->seatcols = $formdata->seatcols;
        }
        if (!empty($formdata->seatblocking)) {
            $sess->seatblocking = $formdata->seatblocking;
        }
        // ecastro ULPGC
    ...
    public function take_from_student($mformdata) : bool {
        ...
        $statuses = array_keys((array)$this->get_statuses());
        $present = reset($statuses); // ecastro ULPGC
        ...
        */
        ...
        if($record->seat &&  $session->seatrows) {
            ...
            $params = array('sid' => $session->id, 'seat' => $record->seat, 'userid' => $USER->id);
            if($DB->record_exists_select('attendance_log', $select, $params)) {
                $url = new moodle_url('/mod/attendance/attendance.php', array('sessid' => $session->id));
                \core\notification::add(get_string('seattaken', 'attendance'), \core\output\notification::NOTIFY_ERROR);
                redirect($url);
            }      
        }
    ...
    public function take_from_form_data($data) {
        ...
        foreach ($formdata as $key => $value) {
            ...
            if (substr($key, 0, 4) == 'seat') {
                ...
                $sesslog[$sid]->seat = $value;
                $sesslog[$sid]->seat = null;
                if(isset($formdata['remarks'.$sid])) {
                    $sesslog[$sid]->remarks = $formdata['remarks'.$sid];
                }
...
//                $sesslog[$sid] = new stdClass();
                ...
                if (array_key_exists('user' . $sid, $formdata) && is_numeric($formdata['user' . $sid])) {
                    ...
                    $sesslog[$sid]->statusid = 0; // ecastro ULPGC
                }
                $sesslog[$sid]->statusset = $statuses;
                $sesslog[$sid]->remarks = $value;
                // ecastro ULPGC
                $sesslog[$sid]->seat = null;
                if(isset($formdata['seat'.$sid])) {
                    $sesslog[$sid]->seat = $formdata['seat'.$sid];
                }
                // ecastro ULPGC
                $sesslog[$sid]->sessionid = $this->pageparams->sessionid;
                $sesslog[$sid]->timetaken = $now;
            ...
            if (!empty($log->statusid) || !empty($log->remarks)) {
                if (array_key_exists($log->studentid, $dbsesslog)) {
                    ...
                    if ($dbsesslog[$log->studentid]->remarks <> $log->remarks ||
                        $dbsesslog[$log->studentid]->seat <> $log->seat ||  // ecastro ULPGC
    ...
    public function get_session_log($sessionid) : array {
        ...
        return $DB->get_records('attendance_log', array('sessionid' => $sessionid), '', 'studentid,statusid,remarks,seat,id,statusset'); // ecastro ULPGC added seat
    }
    ...
    public function get_user_filtered_sessions_log($userid) : array {
        ...
        if ($this->get_group_mode()) {
            // ecastro ULPGC added al.seat & rows
            $sql = "SELECT ats.id, ats.sessdate, ats.groupid, al.statusid, al.remarks, al.seat, ats.seatrows, 
            ...
            // ecastro ULPGC added seat, seatrows
            $sql = "SELECT ats.id, ats.sessdate, ats.groupid, al.statusid, al.remarks, al.seat, ats.seatrows, 
                           ats.preventsharediptime, ats.preventsharedip
                  FROM {attendance_sessions} ats

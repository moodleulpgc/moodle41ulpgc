...
class addsession extends moodleform {
    ...
    public function definition() {
        ...
        // ecastro ULPGC
        if (!empty(get_config('attendance', 'seatingactivitylevel')) && $this->_customdata['att']->seating ) {
            $sizeattr = array('size' => 3);
            $mform->addElement('text', 'seatrows', get_string('seatrows', 'attendance'), $sizeattr);
            $mform->setType('seatrows', PARAM_INT);
            $mform->addHelpButton('seatrows', 'seatrows', 'attendance');        
            if (isset($pluginconfig->seatrows)) {
                $mform->setDefault('seatrows', $pluginconfig->seatrows);
            }
            $mform->addElement('text', 'seatcols', get_string('seatcols', 'attendance'), $sizeattr);
            $mform->setType('seatcols', PARAM_INT);
            $mform->addHelpButton('seatcols', 'seatcols', 'attendance');        
            if (isset($pluginconfig->seatcols)) {
                $mform->setDefault('seatcols', $pluginconfig->seatcols);
            }

            $sizeattr = array('size' => 12);
            $mform->addElement('text', 'seatblocking', get_string('seatblocking', 'attendance'), $sizeattr);            
            $mform->setType('seatblocking', PARAM_TEXT);
            $mform->addHelpButton('seatblocking', 'seatblocking', 'attendance');        
        }
        // ecastro ULPGC

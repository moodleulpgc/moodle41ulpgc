...
function attendance_can_student_mark($sess, $log = true) {
    ...
    if (!empty($attconfig->studentscanmark) && !empty($sess->studentscanmark)) {
        // ecastro ULPGCcheck first seating booking, then standard 
        if($attconfig->seatingactivitylevel && $sess->seatrows) {
            $lastmidnight = usergetmidnight(time()); 
            $end = $lastmidnight + $attconfig->seatbookingadvance*DAYSECS;
            if(($sess->sessdate > $lastmidnight) && ($sess->sessdate < $end)) {
                $canmark = true;
            }
            // ecastro ULPGC

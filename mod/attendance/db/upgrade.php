...
function xmldb_attendance_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2020082501) {
        // ecastro ULPGC
        // Define field seating to be added to attendance.
        $table = new xmldb_table('attendance');
        $field = new xmldb_field('seating', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field timemodified.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }        
        ...
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('seatrows', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field seatrows
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }            
        
        $field = new xmldb_field('seatcols', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0');
        // Conditionally launch add field seatcols.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }            

        $table = new xmldb_table('attendance_log');
        $field = new xmldb_field('seat', XMLDB_TYPE_CHAR, '100', null, null, null, null);
        // Conditionally launch add field seat.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }           
        
        // Attendance savepoint reached.
        upgrade_mod_savepoint(true, 2020082501, 'attendance');
    }
    
    if ($oldversion < 2020082502) {    
        $table = new xmldb_table('attendance_sessions');
        $field = new xmldb_field('seatblocking', XMLDB_TYPE_TEXT, null, null, null, null, null);
        // Conditionally launch add field.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }            
        ...
        // Attendance savepoint reached.
        upgrade_mod_savepoint(true, 2020082502, 'attendance');
    }

...
class viewgrades extends table_sql {
    ...
    protected function init($group = 0, $userid = 0) {
        ...
        if ($CFG->branch < 311) {
            ...
            // ecastro ULPGC
            $fields = \core_user\fields::for_identity($context, false)->excluding([]);
        ...
        if ($this->is_downloading()) {
            ...
            foreach ($extrafields as $field) {
                $fields .= ", u.{$field}";
                $newcolumns[] = $field;
                $newheaders[] = \core_user\fields::get_display_name($field); // for 4.0
            }
            array_splice($tablecolumns, 1, 0, $newcolumns);
            array_splice($tableheaders, 1, 0, $newheaders);
        }

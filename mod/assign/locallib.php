...
if(get_config('local_ulpgcassign', 'enabledadvancedassign')) {
    include_once($CFG->dirroot.'/local/ulpgcassign/lib.php');
}
...
class assign {
    ...
    public function show_intro() {
        ...
        if($this->enabledadvancedassign) { // ecastro ULPGC remove show_intro() (maybe massive) but in main page
            global $PAGE;
            $pagetype = $PAGE->pagetype;
            if($pagetype != 'mod-assign-view' && $pagetype != 'pluginfile') {
                return false;
            }
        }
    ...
    public function get_course_module() {
        ...
        if ($this->context->contextlevel == CONTEXT_MODULE) {
            // ecastro problema de módulos fantasma
            if($modinfo = get_fast_modinfo($this->get_course())) {
                try {
            $this->coursemodule = $modinfo->get_cm($this->context->instanceid);
            return $this->coursemodule;
                } catch (moodle_exception $e) {
                    $cm = get_coursemodule_from_id('assign', $this->context->instanceid);
                    $this->coursemodule = $cm;//cm_info::create($cm);
                    return $this->coursemodule;
                }
            }
        }
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        if ($pluginsubtype == 'assignsubmission') {
            ...
            // Check permissions.
            if(!$allow = optional_param('allow', 0, PARAM_INT)) { // ecastro ULPGC allow access from peers
                if(empty($item->userid) &&  $item->groupid && $this->get_instance()->teamsubmission) { //ecastro ULPGC
            ...
            // Check permissions.
            //$this->require_view_submission($item->userid);
            if($item->userid == 0 &&  $item->groupid && $this->get_instance()->teamsubmission) { //ecastro ULPGC
                $this->require_view_group_submission($item->groupid);
            } else {
                $this->require_view_submission($item->userid);
            }
    ...
    protected function view_footer() {
        ...
        global $SESSION;
        $SESSION->nameformat = ''; // ecastro ULPGC closing naming cache
        if (!PHPUNIT_TEST) {
    ...
    protected function view_grading_table() {
        ...
        if($this->enabledadvancedassign) {
            $gradingoptionsformparams['hidegradingpanel'] = $this->hidegradingpanel;
        }
        ...
        if($this->enabledadvancedassign && $filter) { // ecastro ULPGC to apply advanced filtered message
            $o .= local_ulpgcassign_filtered_warning($filter, $this->get_renderer());
        }
        ...
        if($this->enabledadvancedassign && $filter) { // ecastro ULPGC to apply advanced filtering message
            $tableusers = count($gradingtable->rawdata);
            $filteredout = 0;
            $filteredout = max(0,count($users) - $tableusers);
            $gradingoptionsdata->filtermsg = ($filteredout) ? html_writer::span(get_string('filteredout', 'local_ulpgcassign', $filteredout), 'gradingtablefiltermsg') : null;
            $gradingoptionsform->set_data($gradingoptionsdata);
        }
    ...
    public function get_assign_submission_status_renderable($user, $showlinks) {
        ...
        if($this->enabledadvancedassign) { // ecastro ULPGC advanced assign to add warning if tyeam submission modified after one's submission
            $submissionstatus->modifiedafterwarning = local_ulpgcassign_teamsubmission_after_warning($instance, 
                                                                                                    $submission, 
                                                                                                    $teamsubmission);
        }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        $this->update_submission($mysubmission, 0, true, false); // ecastro ULPGC updatetime = true for all
        // ecastro updatetime this had to be changed because set to false on submit_for_grading for team submission 
        ...
        if (!in_array($submission->status, [ASSIGN_SUBMISSION_STATUS_NEW, ASSIGN_SUBMISSION_STATUS_REOPENED])) {
            foreach ($team as $member) {
                ...
                // ecastro ULPGC to enforce resubmission if other submits after user confirmation
                if($this->enabledadvancedassign && $membersubmission && $this->get_instance()->requireallteammemberssubmit &&  
                                        ($submission->status != ASSIGN_SUBMISSION_STATUS_NEW) && 
                                        ($membersubmission->status == ASSIGN_SUBMISSION_STATUS_SUBMITTED) && 
                                        ($membersubmission->timemodified < $submission->timemodified)) {
                    $DB->set_field('assign_submission', 'status', ASSIGN_SUBMISSION_STATUS_DRAFT, array('id'=>$membersubmission->id));
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if ($submission->status != ASSIGN_SUBMISSION_STATUS_SUBMITTED) {
            ...
            $updatetime = $this->enabledadvancedassign ? !($instance->teamsubmission && 
                                                            $instance->requireallteammemberssubmit) : true; // ecastro ULPGC not updating group sub timemodified on submitting, only on editing submissions
    ...
    protected function process_save_grading_options() {
        ...
        if($this->enabledadvancedassign) {
            $gradingoptionsparams['hidegradingpanel'] = $this->hidegradingpanel; // ecastro ULPGC 
        }
        ...
        if ($formdata = $mform->get_data()) {
            ...
            if (isset($formdata->hidegradingpanel)) {
                set_user_preference('assign_hidegradingpanel', $formdata->hidegradingpanel);
            }

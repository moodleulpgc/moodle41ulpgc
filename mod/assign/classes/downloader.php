...
class downloader {
    ...
    private function load_submissionplugin_filelist(
        ...
        if ($this->downloadasfolders) {
            ...
            $pathfilename = clean_param($pathfilename, PARAM_PATH);
            if($this->enabledadvancedassign  && !$this->instance->teamsubmission || !(in_array($file, $this->filesforzipping))) { // ecastro ULPGC avoid duplicated files for group members in zip file
                $this->filesforzipping[$pathfilename] = $file;
            }

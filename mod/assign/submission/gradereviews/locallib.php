...
class assign_submission_gradereviews extends assign_submission_plugin {
    ...
    public function is_enabled() {
        ...
        || has_capability('assign/submission:canreviewgrade', context_course::instance($COURSE->id))) {
            if (!isset($this->enabledcache)) { // ecastro ULPGC avoid warning
                $this->enabledcache = $this->get_config('enabled');
            }

            return $this->enabledcache && !empty($CFG->usecomments);
        }
     ...
     * @return bool
     */
    public function has_user_summary() { // ecastro ULPGC
        if ($this->is_enabled()) {
            return true;
        }
        return false;

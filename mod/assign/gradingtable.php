...
class assign_grading_table extends table_sql implements renderable {
    ...
    public function __construct(assign $assignment,
        ...
        if($this->assignment->enabledadvancedassign) { // ecastro ULPGC to add support for sorting by groupname
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            ...
            if($this->assignment->enabledadvancedassign) { // adding actual custom filter SQL
                list($where, $params) = local_ulpgcassign_gradingtable_filter_sql($this->assignment->get_instance(), $filter, $where, $params);
            }
        ...
        $allowedui = array();
        $this->newgraderui = false;
        if($this->assignment->enabledadvancedassign && !$this->assignment->hidegradingpanel) { // ecastro ULPGC improve grading interface
            $allowedui = explode(',', get_config('local_ulpgcassign', 'allownewui'));
        }
            ...
            foreach ($this->assignment->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    ...
                    if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->has_user_summary()) {
                        ...
                        $columns[] = $index;
                        $headers[] = $plugin->get_name();
                        if($allowedui && in_array($plugin->get_type(), $allowedui)) { // ecastro ULPGC
                            $this->newgraderui = true;
                        }
                    }
                }
            }
    ...
    public function col_team(stdClass $row) {
        ...
        $status = '';
        if($this->assignment->enabledadvancedassign) { // ecastro ULPGC improve group interface
            if ($submission) {
        ...
        if ($group) {
            return format_string($group->name.' '.$status, true, ['context' => $this->assignment->get_context()]);  //ecastro ULPGC status
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            ...
            if($this->assignment->enabledadvancedassign) { // ecastro ULPGC improve grading interface
                $name = $this->assignment->fullname($row);
                $linknew = '';
                if($this->newgraderui) {
                    $icon = $this->output->pix_icon('gradefeedback',
                                                    get_string('gradeusernewui', 'local_ulpgcassign', $name),
                                                    'mod_assign', array('class'=>'icon'));
                    $linknew = $this->output->action_link($url, $icon);
                }
                ...
                $linkold = $this->output->action_link($url, $icon);
                $link = $linkold;
    ...
    public function col_timesubmitted(stdClass $row) {
        ...
        if ($submission && $submission->timemodified && $submission->status != ASSIGN_SUBMISSION_STATUS_NEW) {
            if($this->assignment->enabledadvancedassign && $submission->status == ASSIGN_SUBMISSION_STATUS_DRAFT) {
                $instance = $this->assignment->get_instance();
                if($this->assignment->enabledadvancedassign && 
                    ...
                    if($membersub = $this->assignment->get_user_submission($row->userid, false)) {
                        $submission->timemodified = $membersub->timemodified;
                    }
                }
            }
    ...
    public function col_status(stdClass $row) {
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if ($this->assignment->enabledadvancedassign && $row->grade !== null && $row->grade >= 0) { // ecastro ULPGC
                $graded = '_graded';
            } else {
                $graded = '';
            }
            $o .= $this->output->container(get_string('submissionstatus_' . $displaystatus, 'assign'),
                                           array('class'=>'submissionstatus' .$displaystatus.$graded)); // ecastro ULPGC
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    ...
                    $now = time();
                    if ($due && ($now > $due) && ($this->assignment->enabledadvancedassign && ($now < $instance->cutoffdate))) { // ecastro ULPGC
    ...
    public function col_userid(stdClass $row) {
        ...
        if($this->assignment->enabledadvancedassign) {
            ...
            if (!$row->grade) {
                $description = get_string('grade', 'grades'); 
            } else {
            ...
            if($this->newgraderui) {
                ...
                if (!$row->grade) {
                    $description = get_string('gradenewui', 'local_ulpgcassign');
                } else {
                    $description = get_string('updategradenewui', 'local_ulpgcassign');
                }

                $actions['grade2'] = new action_menu_link_secondary(
                    $url,
                    $noimage,
                    $description
                );

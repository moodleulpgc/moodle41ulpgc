...
class assignfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        while ($record = $gradeimporter->next()) {
            $user = $record->user;
            $grade =  unformat_float($record->grade);  //$record->grade; // ecastro ULPGC allow grades with comma as decimal separator
            $gradedesc = $grade;
            ...
            if ($usergrade && $usergrade->grade == $grade) {
                ...
                $skip = true;
                      ...
                      (($gradedesc < 0) || ($gradedesc > $assignment->get_instance()->grade))) { // ecastro ULPGC
                // Out of range.
                $skip = true;
            }
            ...
            if (!$skip) {
                ...
                $updates[] = get_string('gradeupdate', 'assignfeedback_offline',
                 ...
                 $updates[] = get_string('gradeignore', 'local_ulpgcassign',
                                            array('grade'=>$gradedesc, 'student'=>$userdesc));  // ecastro ULPGC allow grades with comma as decimal separator
            }

...
// ecastro ULPGC
function offlinequiz_set_course_info($courseorid, &$object) {
    $course = is_numeric($courseorid) ? get_course($courseorid) : $courseorid;
    $object->shortname =  $course->shortname;
    $object->fullname  =  $course->fullname;
    $object->category = '';
    if($category = core_course_category::get($course->category)) {
        $object->category = $category->get_formatted_name();
    }
}

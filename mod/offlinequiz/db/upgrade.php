...
function xmldb_offlinequiz_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2020060802) {    
        // ecastro ULPGC
        // Set table offlinequiz.
        $table = new xmldb_table('offlinequiz');
        
        // Define field questionspercolumn to be added to offlinequiz.
        $field = new xmldb_field('questionspercolumn', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '24', 'fileformat');
        // Conditionally launch add field questionspercolum.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        
        // Define field labelblocks to be added to offlinequiz.
        $field = new xmldb_field('labelblocks', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '1', 'questionspercolumn');
        // Conditionally launch add field questionspercolum.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        
        // Define field qsheetcols to be added to offlinequiz.
        $field = new xmldb_field('qsheetcols', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '1', 'labelblocks');
        // Conditionally launch add field qsheetcols.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        
        // Define field samefformat to be added to offlinequiz.
        $field = new xmldb_field('samefformat', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '1', 'qsheetcols');
        // Conditionally launch add field samefformat.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Offlinequiz savepoint reached.
        upgrade_mod_savepoint(true, 2020060802, 'offlinequiz');        
    }
    
    if ($oldversion < 2020060804) {    
        // ecastro ULPGC
        // Set table offlinequiz.
        $table = new xmldb_table('offlinequiz');
    
        $field = new xmldb_field('questionscols', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '1', 'labelblocks');

        // Launch rename field offlinequiz.
        // Conditionally launch add field samefformat.
        if ($dbman->field_exists($table, $field)) {
            $dbman->rename_field($table, $field, 'qsheetcols');        
        }        
        
        // Offlinequiz savepoint reached.
        upgrade_mod_savepoint(true, 2020060804, 'offlinequiz');
    }  

...
class offlinequiz_rimport_report extends offlinequiz_default_report {
    ...
    public function display($offlinequiz, $cm, $course) {
        ...
        if ($fromform = $importform->get_data() && confirm_sesskey()) {
            ...
            if ($mimetype == 'application/zip') {
                ...
                if ($files) {
                    ...
                    foreach ($files as $file) {
                        ...
                        if ($mimetype == 'application/pdf') {
                            ...
                            $this->extract_multipages($dirname, $dirname . '/' . $file); 
                        }
                    }
                    $files = get_directory_list($dirname);
                } else {
                ...
                */
                // ecastro ULPGC
                $files = $this->extract_multipages($dirname, $importfile); 
     ...
     * @param importfile
     */
    private function extract_multipages($dirname, $importfile, $format = 'png') {
        // Extract each page to a separate file.
        $filename = pathinfo($importfile, PATHINFO_FILENAME);
        $ext = pathinfo($importfile, PATHINFO_EXTENSION);
        $grayscale = '';
        if(strtoupper($ext) == 'PDF') {
            $grayscale = '-type grayscale -density 300';
        }       
        
        $newfile = "$dirname/$filename-%03d.$format";
        $handle = popen("convert $grayscale '$importfile' '$newfile'", 'r');

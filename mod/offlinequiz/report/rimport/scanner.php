...
class offlinequiz_page_scanner {
    ...
    public function __construct($offlinequiz, $contextid, $maxquestions, $maxanswers) {
        ...
        */
        //ecastro ULPGC
        $this->questionspercolumn = $offlinequiz->questionspercolumn;
        $this->labelblocks = $offlinequiz->labelblocks;   
        $this->formtype = offlinequiz_get_formtype($this->questionspercolumn, $maxquestions, $maxanswers);
        $this->questionsonpage = $offlinequiz->questionspercolumn * $this->formtype;
        $this->maxquestions = $maxquestions;
    ...
    public function init_hotspots() {
        ...
        //ecastro ULPGC
        // Load hotspots for userid/moodleid.
        $x= 138 + ($offlinequizconfig->ID_digits) * 6.5;
        $y= 33;
        //$point = new oq_point(1247 + (7.5)*65 - 10 , 306 - 3*60 + 16);
        //$point = new oq_point($x*10 -90 -40, ($y)*10 - 135);
        $point = new oq_point($x*10 - 135, ($y)*10 - 120);
        $this->hotspots['idnumber'] = $point;
        $point = new oq_point($x*10 - 135, ($y+5)*10 - 120);
        $this->hotspots['moodleid'] = $point;
        //ecastro ULPGC
        $x= 106;
        $y = 72;
        $point = new oq_point($x*10 - 145, $y*10 - 120 );          // The box with the cross in the middle.
        $this->hotspots['cross'] = clone $point;         // To get lower trigger.

        $point = new oq_point($x*10 - 144, ($y + 12)*10 - 120);          // The black box in the middle.
        $this->hotspots['deleted'] = clone $point;       // To check if we grabbed it right and to get upper trigger and papergray..

        $colwidth = 7 * 65;
        if ($this->maxanswers > 5) {
            $colwidth = 9 * 65;
        }
        if ($this->maxanswers > 7) {
            $colwidth = 14 * 65;
        }
        if ($this->maxanswers > 12) {
            $colwidth = 26 * 65;
        }
        ...
        */
        ...
        $questionspercolumn = max(5, min(25, $this->questionspercolumn));
        $numperblock = $questionspercolumn;
        if($this->labelblocks) {
            $numperblock = ceil($questionspercolumn/$this->labelblocks);
        }    
        // ecastro ULPGC

        //for ($number = 0; $number < $this->formtype * 24; $number++) {
        for ($number = 0; $number < $this->formtype * ($questionspercolumn); $number++) { // ecastro ULPGC
            ...
            for ($i = 0; $i < $this->maxanswers; $i++) {
                ...
                $point = new oq_point(($i * 65) + ($colwidth * $col) + $offsetx, $y); // ecastro ULPGC
    ...
    public function get_page() {
        ...
        */
         $this->questionsonpage = $this->set_questions_on_page();
    ...
    /**
     * Calculates, stores and returns the number of questions on the current page.
     * @author Enrique Castro @ULPGC  ecastro ULPGC
     *
     */
    public function set_questions_on_page() {
        if ($this->page * $this->formtype * $this->questionspercolumn < $this->maxquestions) {
            //$this->questionsonpage = $this->formtype * 24;
            $this->questionsonpage = $this->formtype * $this->questionspercolumn;
        } else {
            $this->questionsonpage = $this->maxquestions - ($this->formtype * $this->questionspercolumn * ($this->page - 1));
        }
    ...
    public function calibrate($empty, $cross) {
        ...
        $this->upperwarning = $cross + ((100 - $cross) * 0.6); // ecastro ULPGC
    ...
    public function get_usernumber() {
         ...
         */
        if (!$muidvalue && ($offlinequizconfig->ID_digits > $this->iddigits)) { // ecastro ULPGC

...
class mod_offlinequiz_mod_form extends moodleform_mod {
    ...
    protected function definition() {
        ...
        //$options[OFFLINEQUIZ_ODT_FORMAT] = 'ODT'; // ecastro ULPGC
        ...
        // ecastro ULPGC
        $mform->addElement('selectyesno', 'samefformat', get_string('samefformat', 'offlinequiz'), $attribs);
        $mform->addHelpButton('samefformat', "samefformat", 'offlinequiz');
        $mform->disabledIf('samefformat', 'fileformat', 'neq', OFFLINEQUIZ_DOCX_FORMAT);
        
        $options = array_combine(range(5,25), range(5,25));
        $mform->addElement('select', 'questionspercolumn', 
                            get_string('questionspercolumn', 'offlinequiz'), $options, $attribs);
        $mform->addHelpButton('questionspercolumn', 'questionspercolumn', 'offlinequiz');
        $mform->setDefault('questionspercolumn', 10);
        
        $options = array_combine(range(1,4), range(1,4));
        $mform->addElement('select', 'labelblocks', 
                            get_string('labelblocks', 'offlinequiz'), $options, $attribs);
        $mform->addHelpButton('labelblocks', 'labelblocks', 'offlinequiz');                            
        
        $options = array_combine(range(1,2), range(1,2));
        $mform->addElement('select', 'qsheetcols', 
                            get_string('qsheetcols', 'offlinequiz'), $options, $attribs);
        $mform->addHelpButton('qsheetcols', 'qsheetcols', 'offlinequiz');                            
        // ecastro ULPGC
    ...
    public function validation($data, $files) {
        ...
        // ecastro ULPGC
        // Check labelblocks as function of questionspercolumn.
        if( ($data['questionspercolumn'] == 25 && $data['labelblocks'] > 1) || 
            ($data['questionspercolumn'] >= 24 && $data['labelblocks'] > 2) || 
            ($data['questionspercolumn'] >= 21 && $data['labelblocks'] > 3)) {
            $errors['labelblocks'] = get_string('labelblockstoohigh', 'offlinequiz');
        }

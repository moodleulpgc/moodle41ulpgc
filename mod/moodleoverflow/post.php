...
$fileoptions = mod_moodleoverflow_post_form::attachment_options($moodleoverflow);
...
$messageitemid = file_get_submitted_draft_itemid('message');
$fileoptions['maxfiles'] = 5;
file_prepare_draft_area($messageitemid,
    $modulecontext->id,
    'mod_moodleoverflow',
    'post',
    empty($post->id) ? null : $post->id,
...
$postmessage = file_rewrite_pluginfile_urls($postmessage, 'pluginfile.php', 
                        $modulecontext->id, 'mod_moodleoverflow', 'post', $postid); // ecastro ULPGC
...
if ($fromform = $mformpost->get_data()) {
    ...
    $fromform->messagedraftitemid = $fromform->message['itemid']; // ecastro ULPGC
...
if($reply) { 
    ...
    echo moodleoverflow_print_post($post, $discussion, $moodleoverflow, $cm, $course,
        false, false, false, '', '', $postread, true, false, true); // ecastro ULPGC istracked to false
}

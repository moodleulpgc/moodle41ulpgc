...
function moodleoverflow_print_latest_discussions($moodleoverflow, $cm, $page = -1, $perpage = 25) {
    ...
    foreach ($discussions as $discussion) {
        ...
        //ecastro ULPGC discussion blocking// 
...
function moodleoverflow_add_discussion($discussion, $modulecontext, $userid = null) {
    ...
    $post->messageformat = $discussion->messageformat; // ecastro ULPGC
    $post->messagedraftitemid = $discussion-messagedraftitemid; // ecastro ULPGC
    ...
    moodleoverflow_save_editor_files($post, $moodleoverflow, $cm); // ecastro ULPGC
 ...
 * @param object         $discussion     The discussion where to post, eventually // ecastro ULPGC
 *
...
function moodleoverflow_user_can_post($modulecontext, $posttoreplyto, $considerreviewstatus = true, $userid = null) {
    ...
    // Now check if this discussion is open
    // if marksolved, is a teacher not affected by lock
    if(!has_capability('mod/moodleoverflow:marksolved', $modulecontext, $userid)) { 
        // is an student, check for locked
            $moodleoverflow = $DB->get_record('moodleoverflow', ['id' => $posttoreplyto->moodleoverflow], 
                                                                    'id, lockdiscussions, lockuntildate, gradescalefactor', MUST_EXIST);
            if(!\mod_moodleoverflow\ratings::moodleoverflow_discussion_is_locked($moodleoverflow, $posttoreplyto->discussion)) { 
                return false;
            }
    }
            ...
            if (isset($post->children[$aid]->children)) {
                ...
                foreach ($post->children[$aid]->children as $cid => $c) {
                    ...
                    if (isset($post->children[$aid]->children[$cid]->children)) {
                        // ecastro ULPGC NO, don't do that, do NOT clear all posts above level 2.
...
function moodleoverflow_print_post($post, $discussion, $moodleoverflow, $cm, $course,
    ...
    $post->message = file_rewrite_pluginfile_urls($post->message, 'pluginfile.php', 
                        $modulecontext->id, 'mod_moodleoverflow', 'post', $post->id); // ecastro ULPGC
    ...
    if ($canreply) { // ecsatro ULPGC
        ...
        } else if ($iscomment) { // ecastro ULPGC remove ! 
            $replyurl = new moodle_url('/mod/moodleoverflow/post.php#mformmoodleoverflow', array('reply' => $post->id));
...
/**
 * Stores drafiles included in post editor & rewrite post message texto to @@pluginfile@@ format 
 *        absolute links have to be replaced by relative links
 *
 * @param object $post is a full post record, including course and moodleoverflow
 * @param object $moodleoverflow
 * @param object $cm
 ...
 * @return bool
 */
function moodleoverflow_save_editor_files($post, $moodleoverflow, $cm) {
    global $DB;
...
function moodleoverflow_update_post($newpost) {
    ...
    moodleoverflow_save_editor_files($newpost, $moodleoverflow, $cm); // ecastro ULPGC
...
function moodleoverflow_delete_post($post, $deletechildren, $cm, $moodleoverflow) {
    ...
    if ($deletechildren AND $childposts) {
...
function moodleoverflow_update_all_grades_for_cm($moodleoverflowid) {
    ...
    if ($moodleoverflow->grademaxgrade > 0 and $moodleoverflow->gradescalefactor > 0) {
        ...
        foreach ($userids as $userid) {
            ...
            $userrating = \mod_moodleoverflow\ratings::moodleoverflow_get_reputation($moodleoverflow, $userid, true); // ecastro ULPGC faster for tasks

...
/*
        $contentSql = 'SELECT {course_modules}.id as id,
            ...
            ORDER BY taborder;'; 
*/
        $contentSql = 'SELECT cm.id as id,
            cm.visible as visible, 
            t.name as name, 
            t.taborder as taborder,
            t.menuname as menuname,
            cs.section
            FROM ({modules} m INNER JOIN {course_modules} cm ON m.id = cm.module)
            INNER JOIN {tab} t ON cm.instance = t.id 
            JOIN {course_sections} cs ON cs.id = cm.section
            WHERE (((m.name)=\'tab\') AND ((cm.course)=?))
            ORDER BY taborder, cs.section,id;'; //ecastro ULPGC order by ID if same taborder
        ...
        foreach ($results as $result) { /// foreach
            ...
            if ($result->menuname == $this->tab->menuname) {
                ...
                if ($result->visible == 1 || has_capability('moodle/course:update', $this->courseContext)) {
                    ...
                    $i++; // ecastro ULPGC moved here, it seems mustache need zero-based arrays
                }
            }
        }

...
function xmldb_bigbluebuttonbn_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2022080405) { // ecastro ULPGC
        $table = new xmldb_table('bigbluebuttonbn');                                
        $fields = ['completionengagementemojis', 'completionengagementpollvotes', 'completionengagementraisehand', 
                        'completionengagementtalks', 'completionengagementchats'];
        foreach($fields as $name) {
            $field = new xmldb_field($name, XMLDB_TYPE_INTEGER, '9', null, XMLDB_NOTNULL, null, '0', 'completionattendance');
            // Conditionally launch add field remotedatatstamp.
            if (!$dbman->field_exists($table, $field)) {
                $dbman->add_field($table, $field);
            }

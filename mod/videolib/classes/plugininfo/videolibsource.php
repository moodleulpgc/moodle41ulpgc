...
/**
 * Library source subplugin info class.
 *
 * @package     mod_videolib
 * @copyright 2019 Enrique Castro @ ULPGC
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace mod_videolib\plugininfo;
...
class videolibsource extends base {
    /**
    ...
    public static function get_enabled_plugins() {
        ...
        $plugins = core_plugin_manager::instance()->get_installed_plugins('videolibsource');
        if (!$plugins) {
            return array();
        }
        $installed = array();
        foreach ($plugins as $plugin => $version) {
            $installed[] = 'videolibsource_'.$plugin;
        }
    ...
    public static function get_manage_url() {
        return new moodle_url('/mod/videolib/adminmanageplugins.php', array('subtype'=>'videolibsource'));
    }
    ...
    public function load_settings(\part_of_admin_tree $adminroot, $parentnodename, $hassiteconfig) {
        ...
        if ($adminroot->fulltree) {
            $shortsubtype = substr($this->type, strlen('videolib'));

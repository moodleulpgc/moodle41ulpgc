...
function quiz_calculate_best_grade($quiz, $attempts) {
    ...
    switch ($quiz->grademethod) {
        ...
        default:
            if(($quiz->grademethod < -1) && get_config('local_ulpgcquiz', 'enabledadvancedquiz')) { // ecastro ULPGC to allow average of N best
                $max = 0;
                $grades = [0];
                foreach ($attempts as $attempt) {
                    if (!is_null($attempt->sumgrades)) {
                        $grades[] = $attempt->sumgrades;
                    } else {
                        $grades[] = 0;
                    }
                }
                natsort($grades);
                $grades = array_slice(array_reverse($grades), 0, abs($quiz->grademethod));
                $max = array_sum($grades)/abs($quiz->grademethod); 
            } else {
                $max = null;
                foreach ($attempts as $attempt) {
...
function quiz_update_all_final_grades($quiz) {
    ...
    switch ($quiz->grademethod) {
        ...
        case QUIZ_GRADEHIGHEST:
        default:
            if($quiz->grademethod < 0 && get_config('local_ulpgcquiz', 'enabledadvancedquiz')) { // ecastro ULPGC to allow average of N best 
                $select = 'SUM(quiza.sumgrades)/'.abs($quiz->grademethod);
                $join = "JOIN
                            (SELECT GROUP_CONCAT(topNgrades) AS topusergrades 
                                FROM
                                (SELECT SUBSTRING_INDEX(GROUP_CONCAT(qa.id ORDER BY qa.sumgrades DESC), ',',".abs($quiz->grademethod).") AS topNgrades
                                    FROM  {quiz_attempts} qa 
                                    WHERE qa.quiz = {$quiz->id}
                                    GROUP BY qa.userid) AS ids
                            ) AS usergrades ";
                $where = 'FIND_IN_SET(quiza.id, topusergrades) AND ';
            } else {
                $select = 'MAX(quiza.sumgrades)';
                $join = '';
                $where = '';
...
function quiz_get_grading_options() {
    ...
    if(get_config('local_ulpgcquiz', 'enabledadvancedquiz')) { // ecastro ULPGC to allow average best of N grades
        foreach(range(2, 12) as $idx) {
            $options[-$idx] = get_string('bestavgn', 'local_ulpgcquiz', $idx);
        }
    }
    return $options;
}

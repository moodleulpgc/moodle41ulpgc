...
class quizaccess_delayed extends quiz_access_rule_base {
    ...
    protected function configure_timerscript($selector) {
        ...
        // ecastro ULPGC only display if close enough to the open time
        $advance = get_config('quizaccess_delayed', 'timeadvance');
        if($advance && ($this->quiz->timeopen - $this->timenow) > $advance) {
            return;
        } 

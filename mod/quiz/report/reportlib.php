...
function quiz_report_grade_method_sql($grademethod, $quizattemptsalias = 'quiza') {
    switch ($grademethod) {
        ...
        default:
            // ecastro ULPGC
            if($grademethod < 0) {
                return "($quizattemptsalias.state = 'finished' AND $quizattemptsalias.id IN (
                            SELECT SUBSTRING_INDEX(GROUP_CONCAT(qa2.id ORDER BY qa2.sumgrades DESC), ',',".abs($grademethod).") AS topNgrades

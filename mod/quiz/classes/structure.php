...
class structure {
    ...
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            ...
            if($this->canbeedited && 
                            $lock = get_config('quizaccess_makeexamlock')) { // ecastro ULPGC to enforce lock
                global $CFG;
                include_once($CFG->dirroot.'/mod/quiz/accessrule/makeexamlock/lib.php');                
                $this->canbeedited = !quizaccess_makeexamlock_quiz_is_final($this->quizobj->get_quizid());
            }
        }
    ...
    public function get_edit_page_warnings() {
        ...
        if($makeexam = get_config('quiz_makeexam', 'enabled')) { // ecastro ULPGC to enforce examregistrar
            global $CFG;
    ...
    protected function populate_question_numbers() {
        $number = 1;
        foreach ($this->slotsinorder as $slot) {
            if (!isset($this->questions[$slot->questionid]) || $this->questions[$slot->questionid]->length == 0) { // ecastro ULPGC isset()

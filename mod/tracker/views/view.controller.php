...
elseif ($action == 'updateanissue') {
    ...
    //tracker_clearelements($issue->id); // ecastro ULPGC
...
elseif ($action == 'delete') {
    ...
    if(!$confirm = optional_param('confirm', 0, PARAM_INT)) {
        ...
        echo $OUTPUT->header();
        $pageurl = new moodle_url(me());  
        $pageurl->param('confirm', $issueid);
        $pageurl = new moodle_url('/mod/tracker/view.php', $pageurl->params());  
        $notification = $OUTPUT->notification(get_string('issuedeleteconfirm', 'tracker', $issueid), 'notifyproblem');
        echo $OUTPUT->confirm($notification, $pageurl, $url);
        echo $OUTPUT->footer();
        die();
    }
    ...
    foreach ($assignedtokeys as $akey) {
        ...
        if ($haschanged) {
            ...
            if ($oldassign->assignedto != 0) {
                ...
                $ownership->timeassigned = $oldassign->timeassigned ? $oldassign->timeassigned : $now; // ecastro ULPGC
                if (!$DB->insert_record('tracker_issueownership', $ownership)) {
...
elseif ($action == 'cascade') {
    ...
    if (!empty($comments)) {
        ...
        $users = $DB->get_records_select('user', "id $insql", array($inparams), 'lastname, firstname', 'id, firstname, lastname, '.$fields);

        // make backtrack
        foreach ($comments as $comment) {
            $track .= tracker_getstring('commentedby', 'tracker').fullname($users[$comment->userid]).tracker_getstring('on', 'tracker').userdate($comment->datecreated);
            $track .= '<br/>';

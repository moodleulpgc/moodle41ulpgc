...
class scheduler_permissions extends permissions_manager {
    ...
    public function can_edit_slot(\mod_scheduler\model\slot $slot) {
        // ecastro ULPGC
        if($slot->is_attended()) {
            return $this->can_update_attended_slot($slot);
     ...
     * @return bool
     */
    public function can_update_attended_slot(\mod_scheduler\model\slot $slot) {
        if ($this->has_all_capabilities(['manageallappointments', 'editallattended'])) {
            return true;
        } else {
            return ($this->userid == $slot->teacherid) && $this->has_capability('editallattended');
        }
    }

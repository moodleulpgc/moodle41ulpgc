...
class slot extends mvc_child_record_model {
    ...
    public function __construct(scheduler $scheduler) {
        ...
        $this->childrenorder = 'groupid ASC, userid ASC'; //ecastro ULPGC
    ...
    public function get_appointment_count() {
        // ecastro ULPGC
        $scheduler = $this->get_scheduler();
        if($scheduler->is_group_scheduling_enabled() && $scheduler->bookingrouping > 0) {
            global $DB;
            return $DB->count_records_select('scheduler_appointment', 'slotid = ? AND groupid <> 0', array($this->get_id()), 'COUNT(DISTINCT groupid)'); 
        }
        // // ecastro ULPGC
    ...
    private function update_calendar() {
        ...
        $studentids = array();
        $groups = array(0 => array()); // ecastro ULPGC
        $notes = array(); // ecastro ULPGC
        foreach ($myappointments as $appointment) {
            if (!$appointment->is_attended()) {
                //$studentids[] = $appointment->studentid;
                $groups[$appointment->groupid][$appointment->studentid] = $appointment->studentid; // ecastro avoid duplicates
                if($appointment->has_studentnotes()) {
                    $notes[$appointment->groupid][$appointment->studentid] = format_text($appointment->studentnote, $appointment->studentnoteformat);
                }
            }
        }
        
        $teacher = $DB->get_record('user', array('id' => $this->teacherid));
        /* // ecastro ULPGC moved down
        ...
        */
        ...
        $baseevent->description = $schedulername; // ecastro ULPGC
        $baseevent->name = get_string('meetingwith', 'scheduler'); // ecastro ULPGC to avoid failure on updating/deleting events
        $baseevent->format = 1;
        ...
        // ecastro ULPGC update user events         
        if(!empty($groups)) {
            list($insql, $params) = $DB->get_in_or_equal(array_keys($groups));
            $groupnames = $DB->get_records_select_menu('groups', "id $insql", $params, 'name ASC', 'id, name'); 
        }
        $url = new \moodle_url('/user/index.php', array('id'=>$courseid, 'group'=>0));
        foreach($groups as $groupid => $studentids)  {
            ...
            // ecastro ULPGC
            global $USER;
            if(isset($notes[$groupid][$USER->id])) { 
                $studentevent->description .= '<br />'.get_string('field-slotnotes', 'scheduler').'<br />'.$notes[$groupid][$USER->id]; 
            }

            $teacherdescription = '';
            if($groupid) { 
                $url->param('group', $groupid);
                $grouplink = \html_writer::link($url, $groupnames[$groupid]);
                $studentevent->description .= '<br/>'.get_string('asmemberof', 'scheduler', $grouplink);
                $teacherdescription = '<br />'.get_string('group').' '.$grouplink;
            }
            // ecastro ULPGC
            ...
            if (count($studentids) > 0) {
                $teacherids[] = $teacher->id;
                if($groupid) {
                    $teachereventname = get_string('meetingwith', 'scheduler').' '.get_string('group').' '.$groupnames[$groupid];
                } else {
                    // ecastro ULPGC , from above
                    $students = $DB->get_records_list('user', 'id', $studentids);
                    $studentnames = array();
                    foreach ($students as $student) {
                        $studentnames[] = fullname($student);
                    ...
                    if (count($studentids) > 1) {
                        ...
                        $teacherevent->description .= '<br/>'.get_string('students', 'scheduler').', '.implode(', ', $studentnames);
                                        
                    } else {
                        $teachereventname = get_string('meetingwith', 'scheduler').' '.
                                        get_string('student', 'scheduler').', '.reset($studentnames); //$studentnames[0]; // ecastro ULPGC better for id keyed array
                    }
                }
            ...
            // ecastro ULPGC
            if(!empty($notes[$groupid])) { 
                $teacherevent->description .= '<br />'.get_string('field-slotnotes', 'scheduler').'<br />'.implode('<br />', $notes[$groupid]); 
            }
            $teacherevent->description .= $teacherdescription;
            // ecastro ULPGC

...
class scheduler extends mvc_record_model {
    ...
    public function is_individual_scheduling_enabled() {
        if ($this->is_group_scheduling_enabled()) {
            if($this->bookingrouping > 0) { // ecastro ULPGC
                return false; 
            }
    ...
    /**
     * Retrieve whether shared slots are enabled in this instance
     */
    public function shared_slots_enabled() { // ecastro ULPGC
        global $CFG;
        $globalenable = (bool) get_config('mod_scheduler', 'sharedslotsenabled');
        $localenable = $this->data->usesharedslots ;
        return $globalenable && $localenable;
    }
    ...
    protected function fetch_slots($wherecond, $havingcond, array $params, $limitfrom='', $limitnum='', $orderby='') {
        ...
        // ecastro ULPGC
        if($this->shared_slots_enabled()) { // ecastro ULPGC, put in use shared slots
            ...
            $where = " WHERE ( s.schedulerid = :schedulerid  OR ((s.shared > 0 ) AND (s.teacherid $insql ))) ";  // ecastro ULPGC
    ...
    protected function appointment_count_query() {
        ...
        // ecastro ULPGC book only in groups count only groupid ! records
        $sql = "(SELECT COUNT(a.id) FROM {scheduler_appointment} a WHERE a.slotid = s.id)";
        if($this->is_group_scheduling_enabled() && $this->bookingrouping > 0) {
            $sql = "(SELECT COUNT(DISTINCT a.groupid) FROM {scheduler_appointment} a WHERE a.slotid = s.id AND a.groupid <> 0)";
        }
        // ecastro ULPGC end    
    ...
    public function get_slot($id) {
        ...
        // ecastro ULPGC
        $slotdata = $DB->get_record('scheduler_slots', array('id' => $id)); //ecastro ULPGC
        if($slotdata->schedulerid != $this->id  && $slotdata->shared == 0) { // ecastro ULPGC
            throw new moodle_exception('invalidrecord', 'moodle', 'scheduler_slots');
        }
        // ecastro ULPGC
    ...
    public function get_slots_available_to_student($studentid, $includefullybooked = false) {
        ...
        if ($this->groupmode != NOGROUPS) {
            ...
            if ($groups) {
                ...
                $subcond .= " AND (EXISTS ($subquery)"; // ecastro ULPGC
                $params = array_merge($params, $paramsin);
            } else {
                $subcond .= " AND (FALSE"; // ecastro ULPGC
            }
            // ecastro ULPGC add all teacher than can access all groups
            // NOT added in SEPARATEGROUPS && not grouping declared
            $sqlinteacher = '';
            if(!($this->groupmode == SEPARATEGROUPS && !$this->cm->groupingid)             
                 && $teachers = get_users_by_capability($this->get_context(), 'mod/scheduler:attend', 'u.id, u.idnumber',
                                                            '', '', '', '', '', null, null, true)) {        
                list($sqlinteacher, $paramsteacherin) = $DB->get_in_or_equal(array_keys($teachers), SQL_PARAMS_NAMED);
                
                 $subcond .= " OR s.teacherid $sqlinteacher ) ";
                 $params = array_merge($params, $paramsteacherin);
            } else {
                $subcond .= ') ';
            }
            // ecastro ULPGC
    ...
    public function has_slots_booked_for_group($groupid, $mustbeattended = false, $mustbeunattended = false) {
        ...
        $sql = "SELECT COUNT(*)
                 ...
                 WHERE s.schedulerid = :schedulerid
                       AND a.groupid = :groupid AND a.groupid = gm.groupid
                       $attendcond";  // ecastro ULPGC
     ...
     * @param int $slottype the type of slot use (available, appointed, attended etc.)  ecastro ULPGC
     * @return mixed SQL condition and parameters
     */
    protected function slots_for_teacher_cond($teacherid, $groupid, $timerange, $slottype = 3) { // ecastro ULPGC slot type params
        $wheres = array();
        $params = array();
        if ($teacherid > 0) {
        ...
        // ecastro ULPGC
        if($slottype == 0) { // only pending
            $wheres[] = "EXISTS(SELECT 1 FROM {scheduler_appointment} a WHERE a.slotid = s.id AND a.attended = 0)";
            
        } elseif($slottype == 1) {
            $wheres[] = "EXISTS(SELECT 1 FROM {scheduler_appointment} a WHERE a.slotid = s.id AND a.attended = 1)";
        } elseif($slottype == 2) {
            $wheres[] = "NOT EXISTS(SELECT 1 FROM {scheduler_appointment} a WHERE a.slotid = s.id) ";
            
        } elseif($slottype == 3) {
            $wheres[] = "NOT EXISTS (SELECT 1 FROM {scheduler_appointment} a WHERE a.slotid = s.id AND a.attended = 1)";
        }
        // ecastro ULPGC

        if ($groupid > 0) {
            $wheres[] = "((EXISTS (SELECT 1 FROM {groups_members} gm WHERE gm.groupid = :tgid AND gm.userid = s.teacherid)) 
                            OR 
                           (EXISTS (SELECT 1 FROM {scheduler_appointment} a    
                                             JOIN {groups_members} gm ON gm.userid = a.studentid  
                                                WHERE a.slotid = s.id AND gm.groupid = :sgid)) ) ";
            $params['tgid'] = $groupid;
            $params['sgid'] = $groupid;
        }
     ...
     * @return int
     */
    public function count_slots_for_teacher($teacherid, $groupid = 0, $inpast = false, $slottype = 3) { // ecastro ULPGC slottype
        list($where, $params) = $this->slots_for_teacher_cond($teacherid, $groupid, $inpast, $slottype);
     ...
     * @return slot[]
     */
    public function get_slots_for_teacher($teacherid, $groupid = 0, $limitfrom = '', $limitnum = '', $timerange = 0, $slottype = 3) { // ecastro ULPGC slottype
        list($where, $params) = $this->slots_for_teacher_cond($teacherid, $groupid, $timerange, $slottype);
     ...
     * @return slot[]
     */
    public function get_slots_for_group($groupid, $limitfrom = '', $limitnum = '', $timerange = 0, $slottype = 3) { // ecastro ULPGC slottype
        list($where, $params) = $this->slots_for_teacher_cond(0, $groupid, $timerange, $slottype);
        return $this->fetch_slots($where, '', $params, $limitfrom, $limitnum, 's.starttime ASC, s.duration ASC, s.teacherid');
    }
    ...
    public function count_bookable_appointments($studentid, $includechangeable = true) {
        ...
        if($this->is_group_scheduling_enabled() && $this->bookingrouping > 0) {
            $sql .= ' AND  a.groupid <> 0';
            $sql = str_replace('COUNT(*)', 'COUNT(DISTINCT a.groupid)', $sql);
        }
        // ecastro ULPGC end
    ...
    protected function get_available_users($capability, $groupids = 0) {
        ...
        if (is_integer($groupids)) {
            ...
            foreach ($groupids as $groupid) {
                $groupusers = get_enrolled_users($this->get_context(), $capability, $groupid,
                                                 'u.*', null, 0, 0, true); // ecastro ULPGC capability
        ...
        // ecastro ULPGC
        //$enrolled = get_enrolled_users($this->get_context(), $capability, 0, 'u.id, u.idnumber', null, 0, 0, true);
        $accessallgroups = !($this->groupmode == SEPARATEGROUPS && $this->cmid == 0); // to only add teachers with all groups if a grouping declared. So this enforce bookig in common group only 
        $groupid = is_integer($groupids) ? $groupids : 0;
        $users =  array_intersect_key(get_users_by_capability($this->get_context(), $capability, 'u.*', 'u.lastname ASC, u.firstname ASC', '', '',$groupids, '', null, null, $accessallgroups), 
                                            get_enrolled_users($this->get_context(), $capability, $groupid, 'u.id, u.idnumber', null, 0, 0, true));
        // ecastro ULPGC

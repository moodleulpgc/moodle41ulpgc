...
function scheduler_book_slot($scheduler, $slotid, $userid, $groupid, $mform, $formdata, $returnurl) {
    ...
    if ($groupid > 0) {
        ...
        $requiredcapacity = $scheduler->bookingrouping > 0 ? 1 : count($groupmembers); // ecastro ULPGC to allow group as one book
    ...
    foreach ($userstobook as $studentid) {
        ...
        $appointment->groupid = $groupid; // ecastro ULPGC
        $appointment->timecreated = $now; // ecastro ULPGC
        $appointment->timemodified = $now; // ecastro ULPGC
        $appointment->save();
...
/*
        // Notify the teacher.
        if ($scheduler->allownotifications > 0) { // ecastro ULPGC to enforce only cancel notifications
...
*/        
    ...
    // ecastro ULPGC change notifications to teacher
    if($userstobook) {
        // Notify the teacher.
        if ($scheduler->allownotifications > 0) { // ecastro ULPGC to enforce only cancel notifications
            if($groupid > 0) { // if groups, notify with group name
                $student = $DB->get_record('group', array('id' => $groupid), '*', MUST_EXIST);
            } else {
                $student = $DB->get_record('user', array('id' => $appointment->studentid), '*', MUST_EXIST);
            }
            $teacher = $DB->get_record('user', array('id' => $slot->teacherid), '*', MUST_EXIST);
...
if ($action == 'cancelbooking') {
    ...
    // ecastro ULPGC change notifications to teacher, one per group
    if($userstocancel) {
        // Notify the teacher.
        if ($scheduler->allownotifications) { 
            if($appointgroup > 0) { // if groups, notify with group name
                $student = $DB->get_record('group', array('id' => $appointgroup), '*', MUST_EXIST);
            } else {

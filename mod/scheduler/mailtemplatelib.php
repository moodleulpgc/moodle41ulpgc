...
class scheduler_messenger {
    ...
    public static function send_message_from_template($modulename, $messagename, $isnotification,
        ...
        $message->userfrom = core_user::get_noreply_user();  //$sender; // ecastro ULPGC  notifications from no reply
    ...
    public static function get_scheduler_variables(scheduler $scheduler,  $slot,
        ...
        if ($slot) {
            ...
            $vars ['ENDTIME']  = userdate(($slot->starttime + $slot->duration*60), get_string('strftimetime'), $tz);
        ...
        if ($student) {
            ...
            */
            //ecastro ULPGC to allow groupmessages
            if(isset($student->lastname)) { 
                $vars['ATTENDEE']     = fullname($student);
                $vars['ATTENDEE_URL'] = $CFG->wwwroot.'/user/view.php?id='.$student->id.'&course='.$scheduler->course;
            } elseif(isset(($student->name))) {
                $vars['ATTENDEE']     = $student->name;
                $vars['ATTENDEE_URL'] = $CFG->wwwroot.'/user/index.php?id='.$scheduler->course.'&group='.$student->id;
            }
            // ecastro ULPGC
        }
    ...
    public static function send_slot_notification(slot $slot, $messagename, $template,
        ...
        return self::send_message_from_template('mod_scheduler', $messagename, 1, $sender, $recipient, $course, $template, $vars); // ecastro ULPGC
    }

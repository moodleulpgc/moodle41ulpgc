...
function scheduler_print_schedulebox(scheduler $scheduler, $studentid, $groupid = 0) {
    ...
    if ($availableslots) {
        echo $output->box_start(' generalbox  slotchooser'); // ecastro ULPGC
        echo $output->heading(get_string('chooseexisting', 'scheduler'), 3);
        echo $output->render($chooser);
        echo $output->box_end();
    }
    
    return !empty($availableslots); // ecastro ULPGC
}
...
$taburl->param('stype', $slottype);
$baseurl = new moodle_url('/mod/scheduler/view.php', array(
        'id' => $scheduler->cmid,
        'subpage' => $subpage,
        'offset' => $offset,
        'stype' => $slottype // ecastro URL
));
if($pagesize != 25) {
    $taburl->param('perpage', $pagesize);
    $baseurl->param('perpage', $pagesize);
}
...
if ($action == 'updateslot') {
    ...
    $mform = new scheduler_editslot_form($actionurl, $scheduler, $cm, $groupsicansee, array(
            ...
            'groupbook'=>$groupid) // ecastro ULPGC
        );
...
if ($action == 'schedule') {
    ...
    // ecastro ULPGC
        ...
        //echo $output->heading(get_string('scheduleinnew', 'scheduler'), 5);
        echo $output->heading(get_string('scheduleappointment', 'scheduler', fullname($student))); // ecastro ULPGC
        $mform->display();
        echo $output->box_end();
        ...
        $studentid = required_param('studentid', PARAM_INT);
        $student = $DB->get_record('user', array('id' => $studentid), '*', MUST_EXIST);
        $actionurl = new moodle_url($baseurl, array('what'      => 'schedule', 
                                                    'subaction'   => 'newslot',
                                                    'studentid' => $studentid,
                                                    'sesskey'   => sesskey()));
        ...
        echo $output->box_start();
        if(!scheduler_print_schedulebox($scheduler, $studentid)) {
            redirect($actionurl);
        }
        $button = new single_button($actionurl, get_string('scheduleinnew', 'scheduler'));
        $baseurl->param('what', 'view');
        $button2 = new single_button($baseurl, get_string('cancel'));
        echo ' &nbsp; '.$output->render($button).'  &nbsp;  &nbsp;  '.$output->render($button2) ;   
...
if ($action == 'schedulegroup') {
    ...
    if ($subaction == 'dochooseslot') {
        ...
        $mform = new scheduler_editslot_form($actionurl, $scheduler, $cm, $groupsicansee,
                                             array('slotid' => $slotid, 'repeats' => $repeats, 'groupbook' => $groupid));
        $data = $mform->prepare_formdata($slot);
        ...
        $actionurl = new moodle_url($baseurl, array('what' => 'addslot', 'groupid' => $groupid));
        $groupid = required_param('groupid', PARAM_INT);
        ...
        $mform = new scheduler_editslot_form($actionurl, $scheduler, $cm, $groupsicansee, array('repeats' => $i, 'groupbook' => $groupid));
        $mform->set_data($data);
...
if ($DB->count_records('scheduler_slots', array('schedulerid' => $scheduler->id)) <=
    ...
    if ($subpage == 'allappointments') { // ecastro ULPGC
...
if ($groupmode) {
    if ($subpage == 'allappointments') {
        //groups_print_activity_menu($cm, $taburl); ecastro ULPGC already in teacherview_tabs
    } else {
        $a = new stdClass();
...
if($subpage !='schedulestudents' ) { // ecastro ULPGC separate pages
    ...
    $sqlcount = $scheduler->count_slots_for_teacher($teacherid, $slotgroup, false, $slottype); // ecastro ULPGC

    //$pagesize = 25; // ecastro ULPGC. set previously
    if ($offset == -1) {
        if ($sqlcount > $pagesize) {
            $offsetcount = $scheduler->count_slots_for_teacher($teacherid, $slotgroup, true, $slottype); // ecastro ULPGC
    ...
    $slots = $scheduler->get_slots_for_teacher($teacherid, $slotgroup, $offset * $pagesize, $pagesize, false, $slottype); // ecastro ULPGC
    ...
    // ecastro ULPGC slot type selector
    ...
    $options = array(10 => 10,
                    ...
                    50 => 50,
                    100 => 100);
    $select = new single_select($actionurl, 'perpage', $options, $pagesize, array());
    $select->label = get_string('pagesize' ,'scheduler');
    $select->formid = 'selectpagesize';
    $selector .= ' &nbsp; '.$output->render($select);
    echo $output->box($selector, 'selector pagination justify-content-center');
    ...
    if ($slots) {
        ...
        // ecastro ULPGC duplicate paging bar, inverted
...
}// ecastro ULPGC end subpage my/all appointments
...
if($subpage=='schedulestudents') {
    ...
    if ($scheduler->is_group_scheduling_enabled()) {
        ...
        if (empty($groupsicanschedule)) {
            ...
            $groupcnt = 0;
            $groupurl = new moodle_url('/user/index.php', array('contextid'=>$context->get_course_context()->id)); // ecastro ULPGC
            foreach ($groupsicanschedule as $group) {
                ...
                if (!$scheduler->has_slots_booked_for_group($group->id, false, ($scheduler->schedulermode == 'onetime'))) {
                    ...
                    $name = $group->name;
                    // ecastro ULPGC pretyy groupname link
                    $groupurl->param('group', $group->id);
                    $name = html_writer::link($groupurl, $name); 
                    // end ecastro ULPGC
                    $groupmembers = array();
                    foreach ($members as $member) {
                    ...
                    $name .= print_collapsible_region($members, ' groupuserlist ', 'groupuserlist_'.$group->id , ' ', '', true, true); // ecastro ULPGC
                    // ecastro ULPGC
                    $actions = array();
                    $actions[] = new action_menu_link_secondary(
                                    //new moodle_url($actionurl, array('what' => 'schedulegroup', 'groupid' => $group->id)), // ecastro ULPGC

...
class scheduler_editslot_form extends scheduler_slotform_base {
    ...
    protected function definition() {
        ...
        $this->groupbook = 0;
        if (isset($this->_customdata['groupbook'])) {
        ...
        //$grouparray[] = $mform->createElement('hidden', 'groupid', $this->groupbook); // ecastro ULPGC
        ...
        for ($i = 0; $i < $data->appointment_repeats; $i++) {
            if ($data->deletestudent[$i] != 0) {
                ...
                if ($data->appointid[$i]) {
                    ...
                    $app->groupid = $data->groupbook ? $data->groupbook : 0;
                    $app->save();
                }

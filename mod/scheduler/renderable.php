...
class scheduler_student_list implements renderable {
    ...
    /** @var array containhg group structure in student list // ecastro  ULPGC */
    public $groups = array();
    ...
    public function add_student(appointment $appointment, $highlight, $checked = false,
        ...
        if ($showstudprovided) {
            ...
            // ecastro ULPGC to be able to use in students lists
            foreach(array('appointment', 'student', 'teacher') as $field) {
                $name = $field.'note';
                $format = $name.'format';
                $student->{$name} = $appointment->{$name};
                $student->{$format} = $appointment->{$format};
            }
            // ecastro ULPGC
        }
        //$this->students[] = $student;
        // ecastro ULPGC
        $student->groupid = $appointment->groupid;
        $uid = $student->user->id;
        $this->students[$uid] = $student;
        if($student->groupid) {
            if(!isset($this->groups[$student->groupid])) {
                $this->groups[$student->groupid] = array();
            }
            $this->groups[$student->groupid][$uid] = $uid;
        }
        // ecastro ULPGC end
    }
    ...
    public function get_group_names() { // ecastro ULPGC
        global $DB;
        $groups = array();
        if($this->groups) {
            list($insql, $params) = $DB->get_in_or_equal(array_keys($this->groups));
...
class scheduler_slot_manager implements renderable {
    ...
    // ecastro ULPGC
        $slot->schedulerid = $slotmodel->schedulerid; 
        if(isset($slotmodel->data->shared)) { 
            //$slot->etutor = $slotmodel->etutor;
            $slot->shared = $slotmodel->shared;
        }
        if(isset($slotmodel->data->notes)) { 
            //$slot->etutor = $slotmodel->etutor;
            $slot->notes = $slotmodel->notes;
            $slot->notesformat = $slotmodel->notesformat;
        }
     ...
     * @param moodle_url $actionurl action URL for buttons
     */
    public function get_other_course($schedulerid) { // ecastro ULPGC
        global $DB;
        if(!isset($this->othercourses[$schedulerid])) {
            $scheduler = $DB->get_record('scheduler', array('id'=>$schedulerid), 'id, course, name', MUST_EXIST);
            $course = $DB->get_record('course', array('id'=>$scheduler->course), 'id, shortname, fullname', MUST_EXIST);
            $course->instancename = $scheduler->name;
            $this->othercourses[$schedulerid] = $course;
        }
        
        return $this->othercourses[$schedulerid];
    }    
}

...
class mod_scheduler_renderer extends plugin_renderer_base {
    ...
    /**
     * Format the notes relating to an slot or appointment (slotnotes, student, appointment notes and confidential notes) as collapsible text.
     * @copyright Enrique Castro ULPGC  ecastro ULPGC
     *
     * @param scheduler $scheduler the scheduler in whose context the appointment is
     * @param stdClass $data database record describing the slot/appointment
     * @param string $notefield the field in the record containing the notes to foemt. Must be pired with a ..format field
     * @param string $caption stringname for collapsible caption, any of field-x strings
     * @param string $idfield the field in the record containing the item id
     * @param bool $files if area  files should be linked
     * @return string formatted notes
     */
    public function print_collapsible_notes(scheduler $scheduler, $data, $notefield, $caption, $idfield = 'id', $files = false) {
        $note = '';
        $formatfield = $notefield.'format';
        $id = $data->{$idfield};
        $context = $scheduler->get_context();
        if($data->{$notefield}) {
            $note = $this->format_notes($data->{$notefield}, $data->{$formatfield}, $context, $notefield, $id);
            if($files) {
                $note . ' XX ';
                $note .= $this->render_attachments($context->id, $notefield, $id);
            }
            //$caption = get_string('field-'.$caption, 'scheduler');
            $note = print_collapsible_region($note, ' collapsiblenotes ', 'collapsiblenotes_'.$notefield.'_'.$id, $caption, '', true, true);
        }
    ...
    public function teacherview_tabs(scheduler $scheduler, scheduler_permissions $permissions,
        ...
        $level1[] = $this->teacherview_tab($baseurl, 'schedulestudents', 'view', 'schedulestudents'); // ecastro ULPGC
        ...
        // ecastro ULPGC
        $out = $this->tabtree($level1, $selected, $inactive);
        $cm = $scheduler->get_cm();
        if(groups_get_activity_groupmode($cm)) {
            $out .= groups_print_activity_menu($cm, $baseurl, true);
        }
        ...
        foreach ($slottable->slots as $slot) {
            ...
            if ($slottable->showactions) {
                ...
                if ($slot->cancancel) {
                    ...
                    $button->add_confirm_action(get_string('cancelbookingconfirm', 'scheduler', $timedata));
                    $actions .= $this->render($button);
                }
                $rowdata[] = $actions;
            }
    ...
    public function render_scheduler_student_list(scheduler_student_list $studentlist) {
        ...
        if (count($studentlist->students) > 0) {
            ...
            // ecastro ULPGC 
            if($studentlist->groups) {
                $groupnames = $studentlist->get_group_names();
                $url = new moodle_url('/user/index.php', array('id'=>$studentlist->scheduler->get_courseid()));
                foreach($groupnames as $gid => $name) {
                    $url->param('group', $gid);
                    $groupnames[$gid] = html_writer::link($url, $name);
                }
            }
            $groupid = 0;
            $ingroup = false;
            ...
            foreach ($studentlist->students as $student) {
                ...
                if($student->groupid != $groupid) {
                    ...
                    $gtoggleid =  $toggleid.'g'.$groupid;
                    // print group header 
                    $this->page->requires->yui_module('moodle-mod_scheduler-studentlist',
                                    'M.mod_scheduler.studentlist.init',
                                    array($gtoggleid, false));
                    $imgclass = 'studentlist-togglebutton';
                    $alttext = get_string('showparticipants', 'scheduler');
                    $o .= html_writer::div($groupnames[$groupid] . $this->output->pix_icon('t/switch', $alttext, 'moodle',
                                    array('id' => $gtoggleid, 'class' => $imgclass)));
                        $divprops = array('id' => 'list'.$gtoggleid);
                        $o .= html_writer::start_div('studentlist', $divprops);
                        $ingroup = true;
                }
                ...
                if ($student->filesprovided) {
                    ...
                    $alttext = get_string('studentprovided', 'scheduler', $studprovided);
                ...
                if ($student->notesprovided) {
                    ...
                    $alttext = get_string('studentprovided', 'scheduler', $studprovided);
                    $icon = new pix_icon('t/messages', $alttext);
                    $studicons = $this->print_collapsible_notes($studentlist->scheduler, $student, 'studentnote', $this->render($icon).$studicons, 'entryid', true); // ecastro ULPGC
                }
                //ecastro ULPGC 
                ...
                $grade = '';
                // ecastro ULPGC show hint for teacher notes
                $teachernotesalt = array();
                if(isset($student->appointmentnote) && $student->appointmentnote) {
                    $teachernotesalt[] = get_string('appointmentnote', 'scheduler'); 
                } 
                if(isset($student->teachernote) && $student->teachernote) {
                    $teachernotesalt[] = get_string('teachernote', 'scheduler'); 
                }
                if($teachernotesalt) {
                    $icon = new pix_icon('i/manual_item', implode("\n", $teachernotesalt));
                    $grade = $this->render($icon); 
                }
                // ecastro ULPGC show hint for teacher notes
                if ($studentlist->showgrades && $student->grade) {
                    $grade .= $this->format_grade($studentlist->scheduler, $student->grade, true);
                }
                $o .= html_writer::div($checkbox . $picture . ' ' . $name . ' ' . $grade . $studicons, $class); // ecastro ULPGC
            }
    ...
    public function render_scheduler_slot_manager(scheduler_slot_manager $slotman) {
        ...
        //$table->size = array ('2.0rem', '15%', '7.0rem', '6.0rem', '15%', '30%');
        $table->size = array ('1.0rem', '', '', '', '15%', '30%');
        ...
        foreach ($slotman->slots as $slot) {
            ...
            if($slot->shared) { // ecastro ULPGC
                $icon = 'shareto';
                $sharedalt = get_string('sharedto', 'scheduler');
                if($slot->schedulerid != $slotman->scheduler->get_id()) {
                    $icon = 'sharefrom';
                    $other = $slotman->get_other_course($slot->schedulerid);
                    $sharedalt = get_string('sharedfrom', 'scheduler', $other->shortname.': '.$other->instancename);
                }

                $starttimestr .= ' '.$this->render(new pix_icon($icon, $sharedalt, 'scheduler', array('class' => 'sharingicon')));
            }
            ...
            $notes = ($slot->notes) ? $this->print_collapsible_notes($slotman->scheduler, $slot, 'notes', get_string('field-slotnotes', 'scheduler'), 'slotid')  : ''; // ecastro ULPGC
            $rowdata[] = format_string($slot->location).$notes;
            ...
            // ecastroULPGC
            if ($slot->editable && ($slot->isappointed > 1 || count($slot->students->students) > 1)) { 
                // shortcut for marking attended
                $url = new moodle_url($slotman->actionurl, array('what' => 'seenall', 'slotid' => $slot->slotid));
                $actions .= $this->action_icon($url, new pix_icon('i/completion-manual-enabled', get_string('seenall', 'scheduler')));
            }

            if ($slot->isappointed > 1) {
                // shortcut for marking all 
                $url = new moodle_url($slotman->actionurl, array('what' => 'updateslot', 'slotid' => $slot->slotid, 'type'=>'markall'));
                $actions .= $this->action_icon($url, new pix_icon('i/grades', get_string('markall', 'scheduler')));
            }
            // ecastroULPGC

...
class backup_scheduler_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $extra = array('scheduler'=>array(), 'slot'=>array(), 'appointment'=>array());
        if($ulpgc = get_config('local_ulpgccore')) { // ecastro ULPGC
            $extra['scheduler'] = array('usesharedslots', 'completionappointments', 'completionattended');
            $extra['slot'] = array(-1=>'shared');
            $extra['appointment'] = array(-1=>'groupid');
        }
        ...
        $scheduler = new backup_nested_element('scheduler', array('id'), array(
            ...
            'usecaptcha', 'timemodified') + $extra['scheduler']); // ecastro ULPGC
        ...
        $slot = new backup_nested_element('slot', array('id'), array(
            ...
            'emaildate', 'hideuntil') + $extra['slot']); // ecastro ULPGC
        ...
        $appointment = new backup_nested_element('appointment', array('id'), array(
            ...
            'studentnote', 'studentnoteformat', 'timecreated', 'timemodified') + $extra['appointment']); // ecastro ULPGC

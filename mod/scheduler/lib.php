...
function scheduler_delete_instance($id) {
    ...
    // ecastro ULPGC
    $cm = $scheduler->get_cm();
    \core_completion\api::update_completion_date_event($cm->id, 'scheduler', $id, null);
    // ecastro ULPGC
...
function scheduler_get_completion_state($course, $cm, $userid, $type) { // ecastro ULPGC
    ...
    $scheduler = $DB->get_record('scheduler', array('id'=>$cm->instance), '*',
            MUST_EXIST);
    ...
    if ($scheduler->completionattended) {
        $value = $scheduler->completionattended <=
                 $DB->count_records_sql($sql.' AND a.attended = 1 ', array('schedulerid'=>$scheduler->id, 'studentid'=>$userid));
        if ($type==COMPLETION_AND) {
            $result = $result && $value;
        } else {
            $result = $result || $value;
...
function mod_scheduler_core_calendar_provide_event_action(calendar_event $event,
    ...
    $scheduler = $DB->get_record('scheduler', array('id' => $cm->instance));
    $sql = "SELECT COUNT(a.id)
            FROM {scheduler_appointnment} a 
            JOIN {scheduler_slots} s ON s.id = a.slotid 
            WHERE s.schedulerid = :schedid AND a.studentid = :uid ";
    $params = array('schedid' => $scheduler->id, 'userid' => $userid);

    if ($scheduler->completionappointments) {
        $count = $DB->count_records_sql($sql, $params);
        $itemcount += ($scheduler->completionappointments >= $count) ? ($scheduler->completionappointments - $count) : 0;
    }

    if ($scheduler->completionattended) {
        $count = $DB->count_records_sql($sql.' AND a.attended = 1 ', $params);
        $itemcount += ($scheduler->completionattended >= $count) ? ($scheduler->completionattended - $count) : 0;
    }

    // Well there is always atleast one actionable item (view scheduler, etc).
    ...
    return $factory->create_instance(
            ...
            $itemcount, // ecastro ULPGC
            true
    );
}

// ecastro ULPGC added support for completion API 

/**
 * Add a get_coursemodule_info function in case any scheduler type wants to add 'extra' information
...
function scheduler_get_coursemodule_info($coursemodule) {
    ...
    $fields = 'id, name, intro, introformat, completionappointments, completionattended';
     
    if (!$scheduler = $DB->get_record('scheduler', $dbparams, $fields)) {
        return false;
    }
    ...
    if ($coursemodule->completion == COMPLETION_TRACKING_AUTOMATIC) {
        $result->customdata['customcompletionrules']['completionappointments'] = $scheduler->completionappointments;
 ...
 */
function mod_scheduler_get_completion_active_rule_descriptions($cm) {
    ...
    foreach ($cm->customdata['customcompletionrules'] as $key => $val) {
        switch ($key) {
            case 'completionappoitments':
                if (!empty($val)) {
                    $descriptions[] = get_string('completionappoitmentsdesc', 'scheduler', $val);
                }
                break;
            case 'completionattended':
                if (!empty($val)) {
                    $descriptions[] = get_string('completionattendeddesc', 'scheduler', $val);
                }
                break;
            default:
                break;

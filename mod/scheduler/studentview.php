...
if ($scheduler->is_group_scheduling_enabled()) {
    ...
    $mygroupsforscheduling = groups_get_all_groups($scheduler->courseid, $USER->id, $cm->groupingid, 'g.id, g.name'); // ecastro ULPGC
...
if ($scheduler->is_group_scheduling_enabled()) {
    ...
    if($groupmode && array_key_exists($currentgroup, $mygroupsforscheduling) && $appointgroup < $scheduler->bookingrouping) {
        $appointgroup = $currentgroup;
    }
    $canbook = ($scheduler->bookingrouping > 0) ? $canbook && ($appointgroup > 0) : $canbook && ($appointgroup >= 0);
} else {
    $appointgroup = 0;
}
...
// ecastro ULPGC groupmode added in view.php
if ($groupmode) {
    groups_print_activity_menu($cm, $actionurl);
}
...
if (!$canseefull && $bookablecnt == 0) {
    ...
    // Print group selection menu if given.
    if ($scheduler->is_group_scheduling_enabled()) {
        // ecastro ULPGC do not print menu choose if only onne group to book &&
        $choosedots = array(-1 => 'choosedots');
        if($appointgroup && (count($mygroupsforscheduling) > 0)  && $scheduler->bookingrouping > 0) {
            $choosedots = array();
        }
        $groupchoice = array();
        if ($scheduler->is_individual_scheduling_enabled()) {
        ...
        $select = $output->single_select($actionurl, 'appointgroup', $groupchoice, $appointgroup,
                                        $choosedots, 'appointgroupform'); // ecastro ULPGC
        echo html_writer::div(get_string('appointforgroup', 'scheduler', $select), 'dropdownmenu');

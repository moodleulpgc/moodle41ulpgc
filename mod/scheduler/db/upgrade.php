...
function xmldb_scheduler_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2019120200) {
        ...
        $field = new xmldb_field('usesharedslots', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '0', 'usecaptcha');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $table = new xmldb_table('scheduler_slots');
        $field = new xmldb_field('shared', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '0');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $table = new xmldb_table('scheduler_appointment');
        $field = new xmldb_field('groupid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'appointmentnoteformat');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        // Scheduler savepoint reached.
        upgrade_mod_savepoint(true, 2019120200, 'scheduler');
    }

    // ecastro ULPGC 
    if ($oldversion < 2019120200.10) {
        // ULPGC fields
        // Add new configuration fields (relating to booking form) to scheduler.
        $table = new xmldb_table('scheduler');

        $field = new xmldb_field('completionattended', XMLDB_TYPE_INTEGER, '9', null, XMLDB_NOTNULL, null, '0', 'usesharedslots');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        
        $field = new xmldb_field('completionappointments', XMLDB_TYPE_INTEGER, '9', null, XMLDB_NOTNULL, null, '0', 'usesharedslots');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        
        // Scheduler savepoint reached.
        upgrade_mod_savepoint(true, 2019120200.10, 'scheduler');
    }

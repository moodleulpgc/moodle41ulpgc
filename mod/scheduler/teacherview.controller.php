...
function scheduler_action_delete_slots(array $slots, $action, moodle_url $returnurl) {
    ...
    // ecastro ULPGC security, do not delete attended slots
    $caneditall = false;
    if(!empty($slots)) {
        $slot = reset($slots);
        $caneditall = has_capability('mod/scheduler:manageallappointments', $slot->get_scheduler()->get_context()); 
    }
    $keep = 0;
    // ecastro ULPGC
    ...
    // ecastro ULPGC
    if($keep > 0) {
        $message = get_string('attendednondeletable', 'scheduler', $cnt);
...
switch ($action) {
    ...
    case 'deleteslots':
        ...
        $slotids = !empty($slotids) ? explode(",", $slotids) : array(); // ecastro ULPGC avoid fatal error if nothing selected
        $slots = array();

...
$title = get_string('viewconversation', 'mod_dialogue');
...
if ($action == 'create' || $action == 'edit') {
    ...
    if ($form->is_submitted()) {
        ...
        switch ($submitaction) {
            case 'cancel':
                redirect($returnurl);
            case 'sendkeep': $returnurl = new moodle_url('/mod/dialogue/conversation.php',
                                                            array('id' => $cm->id, 'action'=>'view', 'conversationid'=>$conversationid)); // ecastro ULPGC
            case 'send':
                if ($form->is_validated()) {
                    $conversation->save_form_data();
                    $flag = (get_config('dialogue', 'editingtime')) ? \mod_dialogue\dialogue::FLAG_WAITING : null;  // ecastro, to allow delayed  notifications
                    $conversation->send($flag);
    ...
    // test if there are potential partners 
    if(($activityrecord->multipleconversations == 0) && !has_capability('mod/dialogue:openasstaff' , $context)) {  // ecastro ULPGC to enforce nomutipleconversations
        $results = \mod_dialogue\external::potential_users($context, $PAGE->cm);
        if(!$results[0]) {
            redirect($returnurl, get_string('nomatchingpeoplemultiple', 'dialogue'));
        }
    }
    // Display form page.
    echo $OUTPUT->header();

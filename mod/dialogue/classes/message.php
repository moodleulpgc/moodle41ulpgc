...
class message implements \renderable {
    ...
    protected function magic_get_conversationindex() { // ecastro ULPGC
     ...
     * @throws \moodle_exception
     */
    public function send($flag = null) { //ecastro ULPGC to allow delayed notifications
        global $DB;

        if(!$flag) {
            $flag = dialogue::FLAG_SENT; // ecastro ULPGC
        }
        ...
        $subject = $course->shortname.': '.format_string($this->conversation->subject, true, array('context' => $context)); // ecastro ULPGC
        ...
        $posthtml =  "{$course->shortname} -> $strdialogues  ->  ".format_string($this->dialogue->activityrecord->name);   // ecastro ULPGC
        ...
        foreach ($participants as $participant) {
            ...
            $this->set_flag($flag, $participant);

            if($this->dialogue->activityrecord->notifications && $flag == dialogue::FLAG_SENT) { // ecastro ULPGC to allow instance-controlled notifications

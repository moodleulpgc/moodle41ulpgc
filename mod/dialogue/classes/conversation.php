...
class conversation extends message {
    ...
    public function close() {
        ...
        // Permission check.
        $participants = $this->participants ? array_keys($this->participants) : array();
        $canclose = (($this->_authorid == $USER->id) OR  in_array($USER->id, $participants) or has_capability('mod/dialogue:closeany', $context)); // ecastro ULPGC, any cparticipant can close
        if (!$canclose) {
    ...
    public function has_replies($index = 1) { // ecastro ULPGC
        global $DB;
    
        if($this->_replies) {
            foreach($this->_replies as $rid =>$reply) {
                if($reply->conversationindex > $index) {
                    return true;
                }
            }
        } else {
            $select = " conversationid = :conversationid AND state = :state AND conversationindex > :index ";
            $params = array('conversationid' => $this->conversationid, 
                                'state' => dialogue::STATE_OPEN,
                                'index' => $index);
            return $DB->record_exists_select('dialogue_messages', $select, $params);
        }
        return false;
    }

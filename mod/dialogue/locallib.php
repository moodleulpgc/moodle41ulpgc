...
function dialogue_cm_unread_total(\mod_dialogue\dialogue $dialogue) {
    ...
    if($trackunreplied = get_config('dialogue', 'trackunreplied')) { // ecastro ULPGC
        // if confuration asks for replies, rather than unread messages, find and return here
        return dialogue_cm_unreplied_total($dialogue->cm);
    }
...
/**
 * Get a users total unreplied message count for a dialogue course module.
 *
 * @param \mod_info $cm
 * @return int
 */
function dialogue_cm_unreplied_total($cm) {
    global $USER, $DB;
    ...
    // get participant conversations with last mesage author <> user

    $params = array();
    $params['state']       = \mod_dialogue\dialogue::STATE_OPEN;
    $params['lastmessagedialogueid'] = $dialogueid;
    ...
    $sql = "SELECT COUNT(dc.id)
                ...
                JOIN (SELECT dp.conversationid
                        FROM {dialogue_participants} dp
                        WHERE dp.userid = :userid) isparticipant ON isparticipant.conversationid = dc.id

                WHERE dm.authorid <> :authorid ";

    $unreplied = $DB->count_records_sql($sql, $params);
 ...
 * @return string html
 */
function dialogue_generate_summary_line($subject, $body, $bodyformat, $length = 70, $separator = ' - ', $cmid = false,  $cid = false) { // ecastro ULPGC
    if(!$length) {
        $length = 70;
    }
    if(!$separator) {
    ...
    if($cmid) { // ecastro ULPGC
        $url = new moodle_url('/mod/dialogue/conversation.php', array('id'=>$cmid, 'action'=>'view', 'conversationid'=>$cid));
        $len = strlen($subject);
        $subject = html_writer::link($url, $subject);
        $length += (strlen($subject) - $len);
    }
...
/**
* Returns either id or object for penultimate message in conversation by other autho not the very last
*
* @param stdClass $conversation a conversation record
* @return mixed int/stdClass message object
* @author Enrique Castro ULPGC
*/
function get_last_other_participant($conversation) { // ecastro ULPGC    

    global $DB;
    $params = array('dialogueid' => $conversation->dialogueid,
                    'conversationid' => $conversation->conversationid,
                    'authorid' => $conversation->authorid,
                    'state' => $conversation->state);

    $sql = "SELECT dp.id, dp.userid
            FROM {dialogue_participants} dp
            LEFT JOIN {dialogue_messages} dm ON dp.dialogueid = dm.dialogueid
                                                AND dp.conversationid = dm.conversationid
                                                AND dp.userid = dm.authorid
            WHERE dp.dialogueid = :dialogueid AND dp.conversationid = :conversationid
                    AND (dm.authorid IS NULL OR (dm.state = :state AND authorid <> :authorid))
            ORDER BY dp.userid ASC, dm.conversationindex DESC";
    $last = $DB->get_records_sql_menu($sql, $params, 0 ,1);
    if($last) {
        $last = reset ($last);
    }
    return $last;
...
function dialogue_messageswith($dialogue, $own) {
    global $DB, $USER;

    if($own == 'any') {
        $users = $DB->get_records_menu('dialogue_participants', ['dialogueid' => $dialogue->activityrecord->id], '', 'id,userid');
    } else {
        $op = ($own == 'own') ? '=' : '<>';
    
        $sql = "SELECT o.id, o.userid
                FROM {dialogue_participants} p 
                JOIN {dialogue_participants} o ON o.dialogueid = p.dialogueid AND o.conversationid = p.conversationid AND o.userid <> p.userid 
                WHERE p.dialogueid = :did AND p.userid $op :ownid ";
        $users = $DB->get_records_sql_menu($sql, ['did' => $dialogue->activityrecord->id, 'ownid' => $USER->id]); 
    }
    ...
    $users = $DB->get_records_list('user', 'id', array_keys($users), 'lastname', 'id, username, ' . $allusernames);

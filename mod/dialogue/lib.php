...
function dialogue_supports($feature) {
    switch($feature) {
        case FEATURE_GROUPS:
            return true; // ecastro ULPGC reimplement groups
        case FEATURE_GROUPINGS:
            return true; // ecastro ULPGC reimplement groupings
        case FEATURE_MOD_INTRO:
            return true;
        case FEATURE_COMPLETION_TRACKS_VIEWS:
            return true; // ecastro ULPGC reimplement completion
 ...
 */
function dialogue_get_completion_state($course, $cm, $userid, $type) {
    global $CFG,$DB;

    // Get dialogue details
    $dialogue = $DB->get_record('dialogue', array('id'=>$cm->instance), '*',
            MUST_EXIST);

    $result = $type; // Default return value

    if ($dialogue->completionconversations) {
        $value = $dialogue->completionconversations <=
                 $DB->count_records('dialogue_messages',array('dialogueid'=>$dialogue->id,'authorid'=>$userid, 'conversationindex'=>1));
    ...
    $select = " dialogueid = :dialogueid AND authorid = :authorid ";
    if ($dialogue->completionreplies) {
        $value = $dialogue->completionreplies <=
                 $DB->count_records_select('dialogue_messages', $select. " AND conversationindex > :index " ,
                                            array('dialogueid'=>$dialogue->id,'authorid'=>$userid, 'index'=>1));
        if ($type==COMPLETION_AND) {
            $result = $result && $value;
        } else {
            $result = $result || $value;
    ...
    if ($dialogue->completionposts) {
        $value = $dialogue->completionposts <=
                 $DB->count_records_select('dialogue_messages', $select,
                                            array('dialogueid'=>$dialogue->id,'authorid'=>$userid));
        if ($type == COMPLETION_AND) {
            $result = $result && $value;
        } else {
            $result = $result || $value;
 ...
 * @return   bool   true when complete
 */
function dialogue_process_waiting_notifications() { //ecastro ULPGC
    global $CFG, $DB;

    $config = get_config('dialogue');
    if(!$config->editingtime || !$CFG->maxeditingtime) {
        return true;
    }

    mtrace('2. Dealing with waiting notifications ...');

    $cutoff = time() - $CFG->maxeditingtime;
    $select = ' flag = :flag AND timemodified <= :cutoff ';
    $params = array('flag' => \mod_dialogue\dialogue::FLAG_WAITING, 'cutoff'=>$cutoff);
    $rs = $DB->get_recordset_select('dialogue_flags', $select , $params);
    if ($rs->valid()) {
        foreach ($rs as $record) {
            // try and die elegantly
            try {
                ...
                $params = array('id' => $record->messageid, 'conversationid' => $record->conversationid);
                if($DB->record_exists_sql($sql, $params)) {
                    // setup reply
                    $reply = new \mod_dialogue\reply($dialogue, $conversation, (int)$record->messageid);
                    $success = $reply->send(\mod_dialogue\dialogue::FLAG_SENT, false);
                } else {
                    $conversation->add_participant($record->userid);
                    $success = $conversation->send(\mod_dialogue\dialogue::FLAG_SENT, false);
                }

                if($success) {
                    $DB->set_field('dialogue_flags', 'flag', 'mailed', array('id'=>$record->id));
                }
            } catch (moodle_exception $e) {
                mtrace($e->module . ' : ' . $e->errorcode);
            }
    ...
    $sql = "SELECT m.id, m.dialogueid, m.conversationid
            FROM {dialogue_messages} m
            INNER JOIN {dialogue_messages} dm ON m.dialogueid = dm.dialogueid AND m.conversationid = dm.conversationid AND
                                                    m.conversationindex <> dm.conversationindex
            WHERE m.state = :closed AND m.conversationindex = 1 AND dm.state = :open
            GROUP BY m.conversationid ";
    $rs = $DB->get_recordset_sql($sql, array('open'=>\mod_dialogue\dialogue::STATE_OPEN, 'closed'=>\mod_dialogue\dialogue::STATE_CLOSED));
    if ($rs->valid()) {
        foreach ($rs as $record) {
            // try and die elegantly
            try {
                if($success = $DB->set_field('dialogue_messages', 'state', \mod_dialogue\dialogue::STATE_CLOSED, array('dialogueid'=>$record->dialogueid,
                                                                                            'conversationid'=>$record->conversationid))) {

                    mtrace('    closed conversation '.$record->conversationid );
                }
        ...
        mtrace(' None open/closed to process');
    }
    $rs->close();
...
function dialogue_cm_info_view(cm_info $cm) {
    ...
    static $initialised;
    static $usetracking, $strunreadmessagesone, $trackunreplied; // ecastro ULPGC
    if (!isset($initialised)) {
        $trackunreplied = 0;
        if ($usetracking = dialogue_can_track_dialogue()) {
            $read = 'read';
            if($trackunreplied = get_config('dialogue', 'trackunreplied')) {
                $read = 'replied';
            }
            $strunreadmessagesone = get_string("un{$read}messagesone", 'dialogue');
        }
        $initialised = true;
    }
    ...
    if ($usetracking) {
        if($trackunreplied) {
            $read = 'replied';
            $unread = dialogue_cm_unreplied_total($cm);
        } else {
            $read = 'read';
        ...
        if ($unread) {
            ...
            if ($unread == 1) {
                ...
                $out .= get_string("un{$read}messagesnumber", 'dialogue', $unread); // ecastro ULPGC to customize unread/unreplied mesasges
            }
            $out .= '</a></span>';
...
function dialogue_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload = true, $options = array()) {
    ...
    $file = $fs->get_file_by_hash(sha1($fullpath)); // ecastro ULPGC adapt PHP8

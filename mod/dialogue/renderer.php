...
class mod_dialogue_renderer extends plugin_renderer_base {
            ...
            foreach ($list as $record) {
                ...
                if (isset($record->userid)) {
                    ...
                    $html .= html_writer::tag('td', $avatar);
                        ...
                        $fullname = $this->output->action_link($link, fullname($displayuser));
    ...
    /**
     * Builds and returns HTML needed to render the messageswith select form
     *
     * @param \mod_dialogue\dialogue $dialogue
     * @param moodle_url $url PAGE url
     * @return string $html
     */
    public function list_messageswith(\mod_dialogue\dialogue $dialogue, $url) {
        global $CFG, $PAGE, $OUTPUT;

        $users = ['0' => get_string('all')]  + dialogue_messageswith($dialogue, $url->param('own'));
        $selected = $url->param('wu');
        $url->remove_params('wu');

        $html = html_writer::div('', 'clearfix');
        $html .= html_writer::start_div('messageswith pull-right');

        $html .= $this->single_select($url, 'wu', $users, $selected, 
                        null, null, 
                        ['label' => get_string('displaybystudent', 'dialogue')]);
        $html .= html_writer::end_div();

...
if ($hassiteconfig) {
    ...
    // whether to provide unreplied post count // ecastro ULPGC
    $settings->add(new admin_setting_configcheckbox('dialogue/trackunreplied', new lang_string('configtrackunreplied', 'dialogue'),
                   '', 1));

    // whether to provide allow grace period for editing as in forums  / /ecastro ULPGC
    $settings->add(new admin_setting_configcheckbox('dialogue/editingtime', new lang_string('configeditingtime', 'dialogue'),
                   '', 1));
}

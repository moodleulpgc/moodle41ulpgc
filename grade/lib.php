...
class graded_users_iterator {
    ...
    public function init() {
        ...
        if($this->sortfield1) {
            global $SESSION;
            $SESSION->nameformat = $this->sortfield1; // ecastro ULPGC enforce name formating
        }
    ...
    public function close() {
        ...
        if($this->sortfield1) {
            global $SESSION;
            $SESSION->nameformat = ''; // ecastro ULPGC enforce name formating
        }
    }
...
class grade_structure {
    ...
    */
    ...
    public $enabledadvancedgrades = false; // ecastro ULPGC to hold advancedgrading interface setting
            ...
            case 'categoryitem':
                ...
                if ($element['object']->is_calculated()) {
                    ...
                    if ($category = $element['object']->get_item_category()) {
                        ...
                        if(get_config('local_ulpgccore', 'enabledadvancedgrades')) { // ecastro ULPGC grade aggregation icons
                            switch ($category->aggregation) {
                                case GRADE_AGGREGATE_ULPGC_MEAN_EXAM:    
                                case GRADE_AGGREGATE_ULPGC_FINAL:
                                case GRADE_AGGREGATE_ULPGC_MEAN_ACTV:
                                case GRADE_AGGREGATE_ULPGC_MEAN_CONVO:
                                    $icon->pix = 'i/agg_mean';
                                    break;
                                case GRADE_AGGREGATE_ULPGC_SUM:
                                    $icon->pix = 'i/agg_sum';
                                    break;
                            }
                        }
    ...
    public function get_element_header(array &$element, bool $withlink = false, bool $icon = true,
        ...
        // ecastro ULPGC
        if($this->enabledadvancedgrades &&  $element['type'] == 'item' &&   
...
class grade_tree extends grade_structure {
    ...
    public function __construct($courseid, $fillers=true, $category_grade_last=false,
        ...
        $this->enabledadvancedgrades = $fillers ? get_config('local_ulpgccore', 'enabledadvancedgrades') : false; // ecastro ULPGC enforce advanced grading interface
...
abstract class grade_helper {
    ...
    public static function get_aggregation_strings() {
        if (self::$aggregationstrings === null) {
            ...
            if(get_config('local_ulpgccore', 'enabledadvancedgrades')) {  //ecastro ULPGC adding strings for custom aggregations
                self::$aggregationstrings = self::$aggregationstrings + array(
                    GRADE_AGGREGATE_ULPGC_SUM        => get_string('aggregateulpgcsum', 'local_ulpgccore'),
                    GRADE_AGGREGATE_ULPGC_MEAN_ACTV  =>get_string('aggregateulpgcmeanactv', 'local_ulpgccore'),
                    GRADE_AGGREGATE_ULPGC_MEAN_EXAM  =>get_string('aggregateulpgcmeanexam', 'local_ulpgccore'),
                    GRADE_AGGREGATE_ULPGC_MEAN_CONVO =>get_string('aggregateulpgcmeanconvo', 'local_ulpgccore'),
                    GRADE_AGGREGATE_ULPGC_FINAL      =>get_string('aggregateulpgfinal', 'local_ulpgccore'),
                    GRADE_AGGREGATE_ULPGC_NONE       =>get_string('aggregateulpgnone', 'local_ulpgccore'),
                );
            }
        }

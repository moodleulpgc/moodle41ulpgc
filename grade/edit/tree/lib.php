...
class grade_edit_tree {
    ...
    public function __construct($gtree, $moving, $gpr) {
        ...
        $this->gradebooklocked = false;
        if(get_config('local_ulpgccore', 'enabledgradebooklocking')) { // ecastro ULPGC enforce gradebook locking
            $context = context_course::instance($COURSE->id);
            if(!has_capability('local/ulpgccore:manage', $context)) { 
                $this->gradebooklocked = $context;
            }
            $this->gradebooklockingdepth = get_config('local_ulpgccore', 'enabledgradebooklocking');
        }
        ...
        if ($element['type'] == 'category') {
            ...
            foreach($element['children'] as $child_el) {
                ...
                if ($this->moving && $this->moving != $child_eid) {
                    ...
                    // ecastro ULPGC to enforce gradebook locking: do not move into depthlocked categories
                    if($this->gradebooklocked) {
                        if($after_el = $this->gtree->locate_element($child_eid)) {
                            $parent = $after_el['object']->parent_category; 
                            if(!$parent || ($parent->depth < $this->gradebooklockingdepth) || (($parent->depth == $this->gradebooklockingdepth) && !$first)) {
                                $moveto = null;
                            }
                        }
                    }
                ...
                if ($this->moving && ($child_el['object']->itemtype == 'course' || $child_el['object']->itemtype == 'category') && !empty($moveto)) {  // ecastro ULPGC moveto to enforce moving lock
    ...
    function element_deletable($element) {
        ...
        if($this->gradebooklocked && ($element['type'] == 'category')) { // ecastro ULPGC enforce gradebook locking items
            return local_ulpgccore_gradecat_deletable($element['object'], $this->gradebooklocked);
        }

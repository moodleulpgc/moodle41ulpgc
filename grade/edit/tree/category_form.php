...
/// parent category related settings
        ...
        foreach ($categories as $cat) {
            ...
            if($cat->depth >= $depth) { // ecastro ULPGC enforce not move into locked
...
/// tweak the form - depending on existing data
    function definition_after_data() {
        ...
        if ($id = $mform->getElementValue('id')) {
            ...
            // ecastro ULPGC to enforce gradebook locking
            if($ulpgc = get_config('local_ulpgccore', 'enabledgradebooklocking')) { // ecastro ULPGC
                $freezed = array();
                $context = context_course::instance($COURSE->id);
                if($locked = local_ulpgccore_gradecat_locked($grade_category, $context)) {
                    $freezed[] = 'grade_item_iteminfo';
                    if($locked >= GRADECAT_LOCKED_AGG) {
                        $freezed[] = 'aggregation';
                    }
                    if($locked == GRADECAT_LOCKED_NAME || $locked == GRADECAT_LOCKED_BOTH) {
                            $freezed[] = 'fullname';
                            $freezed[] = 'grade_item_idnumber';
                            $freezed[] = 'grade_item_itemname';
                    }
                }

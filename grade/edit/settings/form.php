...
class course_settings_form extends moodleform {
    ...
    function definition() {
        ...
        if($ulpgc = get_config('local_ulpgccore', 'enabledadvancedgrades')) { // ecastro ULPGC put in use detailed scales
            $options = array(-1                           => get_string('default', 'grades'),
                            GRADE_NORMAL_SCALE_DISPLAY   => get_string('normalscaledisplay', 'local_ulpgccore'),
                            GRADE_DETAILED_SCALE_DISPLAY => get_string('detailedscaledisplay', 'local_ulpgccore'));
            $default_scaledisplaymode = GRADE_DEFAULT_SCALE_DISPLAY;
            foreach ($options as $key=>$option) {
                if ($key == $default_scaledisplaymode) {
                    $options[-1] = get_string('defaultprev', 'grades', $option);
                    break;
                }
            }
            $mform->addElement('select', 'scaledisplaymode', get_string('scaledisplaymode', 'local_ulpgccore'), $options);
            $mform->addHelpButton('scaledisplaymode', 'scaledisplaymode', 'local_ulpgccore');
        }

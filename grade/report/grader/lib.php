...
class grade_report_grader extends grade_report {
    ...
    public function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
        ...
        if($this->sortitemid == 'firstname' || $this->sortitemid == 'lastname') {  // ULPGC ecastro to allow listing users by lastname
            global $SESSION;
            $SESSION->nameformat = $this->sortitemid;
        }
    }
                ...
                if ($item->needsupdate) {
                    ...
                    if ($item->needsupdate) {
                        ...
                        if (get_config('local_ulpgccore', 'scaledisplaymode')) { // ecastro ULPGC apply detailed scales
                            $detailedscales = local_ulpgccore_format_gradevalue($gradeval, $item, $userid);
                        }
                        $itemcell->text .= $gradepassicon . "<span class='gradevalue{$hidden}{$gradepass}'>" .
                                grade_format_gradevalue($gradeval, $item, true, $gradedisplaytype, null) . " $detailedscales</span>";
    ...
    public function get_grade_table($displayaverages = false) {
        ...
        global $SESSION;
        $SESSION->nameformat = ''; // ecastro ULPGC namiing formatting
